...
const Section = ({ icon, title, accent = false, children }) => (
  <div
    className={`bg-white rounded-xl border-2 ${accent ? "border-[#F97316]" : "border-slate-200"} overflow-hidden`}
  >
    <div
      className={`flex items-center gap-3 px-5 py-3 border-b-2 ${accent ? "border-[#F97316] bg-[#FFF7ED]" : "border-slate-100 bg-slate-50"}`}
    >
      <span
        className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${accent ? "bg-[#F97316]" : "bg-[#1E3A8A]"}`}
      >
...
const Input = ({ prefix, suffix, className = "", ...rest }) => (
  <div
    className={`flex items-center border-2 border-slate-200 rounded-lg bg-white overflow-hidden focus-within:border-[#1D4ED8] transition-colors ${className}`}
  >
    {prefix && (
      <span className="px-3 py-2.5 bg-slate-50 border-r-2 border-slate-200 text-slate-500 text-sm font-semibold shrink-0">
        {prefix}
      </span>
    )}
    <input
    ...
    />
    {suffix && (
      <span className="px-3 py-2.5 bg-slate-50 border-l-2 border-slate-200 text-slate-500 text-xs font-semibold shrink-0">
        {suffix}
      </span>
    )}
  </div>
...
const Select = ({ children, ...rest }) => (
  <select
    {...rest}
    className="w-full border-2 border-slate-200 rounded-lg px-3 py-2.5 text-sm text-[#1E293B] outline-none bg-white focus:border-[#1D4ED8] transition-colors font-['Barlow',sans-serif]"
  >
    {children}
  </select>
);
...
const EditProduct = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setForm({
          ...
          holcellMargin: p.holcellMargin ?? 3,
          retailMargin: p.retailMargin ?? 5,
      ...
      .catch(() => {
        setToast({ type: "error", msg: "Failed to load product." });
        setLoading(false);
      });
  ...
  const holcell =
    buying + buying * (parseFloat(form?.holcellMargin) / 100 || 0);
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      form.isCustom &&
      !assignSupplierId &&
      !(assignOther && assignOtherName.trim())
    ) {
      setToast({
        type: "error",
        msg: "Assign a real supplier before converting this to a regular product.",
      });
      setTimeout(() => setToast(null), 4000);
      return;
    }
    ...
    try {
      ...
      setToast({
        type: "error",
        msg: error.response?.data?.message || "Failed to update product.",
      });
    } finally {
      setSaving(false);
    }
  ...
  return (
    <>
      ...
      {toast && (
        <div
          className={`fixed top-5 right-5 z-50 flex items-center gap-3 px-5 py-3 rounded-xl border-2 text-sm font-semibold font-['Barlow',sans-serif] ${
            toast.type === "success"
      ...
      <form
        ...
        className="flex flex-col gap-5 font-['Barlow',sans-serif] p-20 mx-auto"
      >
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => navigate(-1)}
              className="w-9 h-9 rounded-lg border-2 border-slate-200 flex items-center justify-center text-slate-500 hover:border-[#1E3A8A] hover:text-[#1E3A8A] transition-colors"
            >
              <FiArrowLeft size={16} />
            </button>
          ...
          <button
            type="submit"
            disabled={saving}
            className="flex items-center gap-2 px-5 py-2 rounded-lg border-2 border-[#F97316] bg-[#F97316] text-white text-sm font-bold hover:bg-[#EA6C0A] hover:border-[#EA6C0A] transition-colors disabled:opacity-60"
          >
            {saving ? (
              "Saving..."
            ) : (
          ...
          <div className="flex items-start gap-3 bg-amber-50 border-2 border-amber-300 rounded-xl px-4 py-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-700 mt-0.5">
                It was created directly from an invoice. Fill in the details
                below and assign a real supplier to convert it into a regular
                catalog product.
              </p>
            </div>
          </div>
          ...
          <div className="xl:col-span-2 flex flex-col gap-5">
            ...
            <Section icon={<FiDollarSign />} title="Pricing">
              ...
              <div className="flex gap-3 mt-3 text-xs font-semibold text-slate-500">
                <span>
                  Wholesale:{" "}
                  <span className="text-[#1D4ED8]">৳{holcell.toFixed(2)}</span>
                </span>
                <span>
                  Retail:{" "}
                  <span className="text-[#F97316]">৳{retail.toFixed(2)}</span>
                </span>
              </div>
            </Section>
            ...
            <Section icon={<FiArchive />} title="Stock & Inventory">
              ...
              <p className="text-[10px] text-slate-400 font-medium">
                * Current stock is managed via Restock / Sales and isn't edited
                here.
              </p>
            </Section>
            ...
            {form.isCustom && (
              <Section icon={<FiTruck />} title="Assign Real Supplier" accent>
                <p className="text-xs text-slate-400 mb-3">
                  This product's stock is currently marked "N/A (Custom
                  Product)". Assign a real supplier to convert it into a regular
                  product.
                </p>
                <Select
                  ...
                  onChange={(e) => {
                    if (e.target.value === "__other__") {
                      ...
                      setAssignSupplierId(e.target.value);
                    }
                  }}
                >
          ...
          <div className="flex flex-col gap-5">
            <Section icon={<FiInfo />} title="Status">
              <div className="flex flex-col gap-3">
                {["active", "inactive", "discontinued"].map((s) => (
                  <label
                    key={s}
                    className={`flex items-center gap-3 px-4 py-3 rounded-lg border-2 cursor-pointer transition-colors ${form.status === s ? "border-[#1D4ED8] bg-[#EFF6FF]" : "border-slate-200 bg-white hover:border-slate-300"}`}
                  >
                    <input
                      type="radio"
                      name="status"
            ...
            <Section icon={<FiImage />} title="Product Images">
              <div
                ...
                className="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center cursor-pointer hover:border-[#F97316] hover:bg-[#FFF7ED] transition-colors"
              >
              ...
              {(existingImages.length > 0 || images.length > 0) && (
                <div className="grid grid-cols-2 gap-2 mt-3">
                  {existingImages.map((img, i) => (
                    <div
                      key={"e" + i}
                      className="relative rounded-lg border-2 border-slate-200 overflow-hidden aspect-square bg-slate-50"
                    >
                      <img
                        src={img}
                        alt=""
                      ...
                      <button
                        ...
                        className="absolute top-1 right-1 w-6 h-6 bg-red-500 text-white rounded-md flex items-center justify-center hover:bg-red-600"
                      >
                        <FiX size={12} />
                      </button>
                    </div>
