...
const avatarBg = (name) =>
  COLORS[name?.charCodeAt(0) % COLORS.length] || "bg-green-600";
const initials = (n) =>
  n
    ?.split(" ")
    .map((w) => w[0])
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center px-4">
      <div className="bg-white rounded-3xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="flex items-center justify-between px-7 py-5 border-b border-[#F1F5F9]">
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-[#F1F5F9] rounded-xl transition"
          >
        ...
        <div className="px-7 pb-6 flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 border border-[#E2E8F0] text-[#334155] font-bold py-3.5 rounded-xl text-sm hover:bg-[#F8FAFC] transition"
          >
            Cancel
          </button>
          <button
            onClick={handleSave}
            className="flex-1 bg-[#16A34A] hover:bg-[#15803D] text-white font-bold py-3.5 rounded-xl text-sm shadow-[0_4px_14px_-2px_rgba(22,163,74,0.35)] transition"
          >
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-sm bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
        ...
        <div className="px-6 py-6 flex items-center gap-4 border-b border-[#F1F5F9]">
          <div className="w-16 h-16 rounded-full bg-[#334155] flex items-center justify-center text-white text-xl font-extrabold">
            {initials(c.name)}
          </div>
          <div>
            ...
            <span
              className={`inline-block text-xs px-3 py-0.5 rounded-full font-bold mt-1 ${(c.totalDue || 0) > 0 ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}`}
            >
          ...
          {[
            {
              ...
              label: "Phone",
              val: c.phone || "Not provided",
            },
            {
              icon: <FiMail size={15} />,
              label: "Email",
              val: c.email || "Not provided",
            },
            {
            ...
            },
            {
              ...
              label: "Joined",
              val: fmtDate(c.createdAt),
            },
            {
              icon: <FiCalendar size={15} />,
              label: "Last Order",
              val: c.lastOrderDate ? fmtDate(c.lastOrderDate) : "No orders yet",
            },
          ].map(({ icon, label, val }) => (
            <div key={label} className="flex items-start gap-3">
              ...
              <div>
                ...
                <div
                  className={`text-sm font-semibold ${val === "Not provided" ? "text-[#94A3B8] italic" : "text-[#0F172A]"}`}
                >
                  {val}
                </div>
              </div>
        ...
        <div className="px-6 py-5 grid grid-cols-2 gap-3">
          <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4">
            <div className="text-emerald-700 text-lg font-extrabold">
              {fmt(c.totalSpent || 0)}
            </div>
            <div className="text-emerald-600 text-xs mt-0.5 font-semibold">
              Total spent
            </div>
          </div>
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
            <div className="text-blue-700 text-lg font-extrabold">
              {c.totalOrders || 0}
            </div>
            <div className="text-blue-600 text-xs mt-0.5 font-semibold">
              Orders
            </div>
          </div>
          <div
            className={`${(c.totalDue || 0) > 0 ? "bg-red-50 border-red-100" : "bg-[#F8FAFC] border-[#F1F5F9]"} border rounded-xl p-4 col-span-2`}
          >
            <div
              className={`text-lg font-extrabold ${(c.totalDue || 0) > 0 ? "text-red-600" : "text-[#94A3B8]"}`}
            >
              {(c.totalDue || 0) > 0 ? fmt(c.totalDue) : "No due ✅"}
            </div>
            <div className="text-xs mt-0.5 text-[#94A3B8] font-semibold">
              Outstanding due
            </div>
          </div>
        </div>
        <div className="mt-auto px-6 pb-6 flex gap-3">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl text-sm transition"
          >
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 bg-red-500 hover:bg-red-600 text-white font-bold py-3 rounded-xl text-sm transition"
          >
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans flex">
      ...
      <div className="flex-1 min-w-0">
        {toast.msg && (
          <div
            className={`fixed top-5 right-5 z-50 px-5 py-3 rounded-2xl shadow-xl text-sm font-bold flex items-center gap-2 text-white ${toast.type === "success" ? "bg-[#16A34A]" : "bg-red-500"}`}
          >
          ...
          <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center px-4">
            <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
              ...
              <p className="text-[#64748B] text-sm mb-6">
                This customer will be removed permanently.
              </p>
        ...
        <div className="max-w-375 mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
          {/* Breadcrumb */}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-[#16A34A] rounded-2xl flex items-center justify-center text-white shadow-[0_4px_14px_-2px_rgba(22,163,74,0.35)]">
                <FiUsers size={22} />
              </div>
              <div>
                ...
                <p className="text-[#94A3B8] text-sm mt-0.5">
                  Every customer in your system
                </p>
              </div>
            </div>
            <button
              onClick={() => setModal("add")}
              className="flex items-center gap-2 bg-[#16A34A] hover:bg-[#15803D] text-white font-bold px-5 py-3 rounded-xl text-sm shadow-[0_4px_14px_-2px_rgba(22,163,74,0.35)] transition"
            >
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
            {[
              ...
              <div
                key={label}
                className="bg-white border border-[#F1F5F9] rounded-2xl p-4 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]"
              >
                <div className="flex items-center gap-2">
                  <span
                    className={`w-7 h-7 rounded-lg flex items-center justify-center text-white shrink-0 ${bg}`}
                  >
                    <Icon size={13} />
                  </span>
                ...
                <div className="flex items-end gap-2 mt-2">
                  <div className="text-xl font-extrabold text-[#0F172A] leading-none">
                    {val}
                  </div>
                  {trend && (
                    <span className="text-[10px] font-bold text-[#16A34A] mb-0.5">
                      {trend}
                    </span>
                  )}
                </div>
                      ...
                      <tr key={c._id} className="hover:bg-[#F8FAFC] transition">
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-full bg-[#334155] flex items-center justify-center text-white text-xs font-bold shrink-0">
                              {initials(c.name)}
                            </div>
                            <div>
                        ...
                        <td className="px-5 py-4 text-sm text-[#64748B]">
                          <span className="truncate block max-w-35">
                            {c.address || "—"}
                          </span>
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() => setModal(c)}
                              className="p-2 bg-emerald-50 hover:bg-emerald-100 text-[#16A34A] rounded-lg transition"
                            >
                              <FiEdit2 size={15} />
                            </button>
                            <button
                ...
                pagedData.map((c) => (
                  <div key={c._id} className="p-5">
                    <div className="flex items-start justify-between gap-2 mb-4">
                      <div className="flex items-center gap-3">
                        <div className="w-11 h-11 rounded-full bg-[#334155] flex items-center justify-center text-white font-bold text-sm">
                          {initials(c.name)}
                        </div>
                        <div>
                      ...
                      <span
                        className={`text-[11px] font-bold px-2.5 py-1 rounded-full capitalize ${(c.totalDue || 0) > 0 ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}`}
                      >
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-4">
                      ...
                      <div
                        className={`${(c.totalDue || 0) > 0 ? "bg-red-50" : "bg-[#F8FAFC]"} rounded-xl p-3 text-center`}
                      >
                        <div
                          className={`text-xs font-bold ${(c.totalDue || 0) > 0 ? "text-red-600" : "text-[#94A3B8]"}`}
                        >
                          {(c.totalDue || 0) > 0 ? fmt(c.totalDue) : "—"}
                        </div>
                        <div className="text-[10px] text-[#94A3B8] mt-0.5 font-medium">
                          Due
                        </div>
          ...
          <p className="text-center text-[#94A3B8] text-sm pb-4">
            Khulna Hardware Mart · All Customers
          </p>
        </div>
      </div>
    </div>
