...
import {
  ...
  FiGrid, FiBox, FiUsers, FiShoppingCart, FiBarChart2, FiSettings,
  FiChevronRight, FiChevronLeft, FiBell, FiSmartphone, FiCreditCard,
} from "react-icons/fi";
...
const NAV_ITEMS = [
  ...
  { key: "customers", label: "Customers", icon: FiUsers, active: true, children: ["Orders", "Due Customers", "Limit Managers"] },
...
function toCsv(rows) {
  const headers = ["Name", "Phone", "Email", "Address", "Customer Type", "Orders", "Total Spent", "Due Amount", "Joined", "Last Order", "Invoice Count"];
  const escape = (val) => {
    const s = String(val ?? "");
  ...
  const lines = rows.map((c) => [
    ...
    c.invoiceCount, c.totalSpent, c.totalDue, fmtDate(c.joinedAt), fmtDate(c.lastOrderDate), c.invoiceCount,
  ].map(escape).join(","));
...
function Drawer({ c, onClose, onPreview }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoading(true);
    axios.get("http://localhost:5000/api/invoices", { params: { customerName: c.name, paymentStatus: "due", limit: 100 } })
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-sm bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
        ...
        <div className="relative bg-[#1E3A8A] pt-8 pb-14 px-6">
          <button onClick={onClose} className="absolute top-4 right-4 p-1.5 bg-white/15 hover:bg-white/25 rounded-full transition">
            <FiX size={16} className="text-white" />
          </button>
          <div className="flex flex-col items-center">
            <div className={`w-20 h-20 rounded-full ring-4 ring-white/30 flex items-center justify-center text-white text-2xl font-extrabold ${avatarBg(c.customerId || c.name)}`}>
              {initials(c.name)}
            </div>
            ...
            <div className="flex items-center gap-1.5 mt-1.5">
              ...
              <span className={`text-[11px] px-2.5 py-0.5 rounded-full font-bold bg-white/90 ${typeStyle(c.customerType).split(" ")[1]}`}>
        ...
        <div className="px-6 -mt-8">
          <div className="bg-white rounded-2xl border border-[#F1F5F9] shadow-[0_8px_24px_-6px_rgba(15,23,42,0.12)] p-4 grid grid-cols-2 gap-3">
            <div>
              <p className="text-[10px] font-bold text-[#94A3B8] uppercase">Personal Info</p>
              <p className="text-sm font-bold text-[#0F172A] mt-0.5 truncate">{c.phone}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold text-[#94A3B8] uppercase">Email</p>
              <p className="text-sm font-bold text-[#0F172A] mt-0.5 truncate">{c.email || "—"}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold text-[#94A3B8] uppercase">Join Date</p>
              <p className="text-sm font-bold text-[#0F172A] mt-0.5">{fmtDate(c.joinedAt)}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold text-[#94A3B8] uppercase">Last Order</p>
              <p className="text-sm font-bold text-[#0F172A] mt-0.5">{fmtDate(c.lastOrderDate)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 mt-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3.5">
              <p className="text-[#0F172A] text-base font-extrabold">{fmt(c.totalSpent)}</p>
              <p className="text-emerald-600 text-[11px] font-semibold mt-0.5">Total Spent</p>
            </div>
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-3.5">
              <p className="text-[#0F172A] text-base font-extrabold">{c.invoiceCount}</p>
              <p className="text-blue-600 text-[11px] font-semibold mt-0.5">Total Orders</p>
            </div>
          </div>
          <div className="bg-red-500 rounded-xl p-4 mt-3 flex items-center justify-between">
            <div>
              <p className="text-red-100 text-[11px] font-bold uppercase">Outstanding Due Amount</p>
              <p className="text-white text-xl font-extrabold mt-0.5">{fmt(c.totalDue)}</p>
            </div>
        ...
        <div className="px-6 pt-5 pb-2">
          <a href={`tel:${c.phone}`} className="w-full flex items-center justify-center gap-2 bg-[#16A34A] hover:bg-[#15803D] text-white font-bold py-3 rounded-xl text-sm shadow-[0_4px_14px_-2px_rgba(22,163,74,0.35)] transition">
        ...
        <div className="px-6 py-5">
          <p className="text-sm font-extrabold text-[#0F172A] mb-4 flex items-center gap-1.5"><FiFileText size={14} /> Invoice History</p>
          {loading ? (
              ...
              {invoices.map((inv) => (
                <div key={inv._id} className="relative">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-bold text-[#0F172A]">{inv.invoiceNumber}</p>
                      <p className="text-[11px] text-[#94A3B8]">{inv.invoiceDate}</p>
                    </div>
...
export default function DueCustomers() {
  ...
  const filteredRows = useMemo(() => {
    let list = data;
    if (typeFilter !== "all") list = list.filter((c) => String(c.customerType) === String(typeFilter));
    return list;
  }, [data, typeFilter]);
  ...
  const toggleSelectAll = () => setSelected((p) => p.length === rows.length ? [] : rows.map((c) => c.customerId || c.name));
      ...
      <div className="flex-1 min-w-0">
          ...
          <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center px-4">
            <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
              ...
              <p className="text-[#64748B] text-sm mb-6">
                <strong className="text-[#0F172A]">{data.find((c) => (c.customerId || c.name) === delId)?.name}</strong> will be removed.
              </p>
          ...
          <div className="fixed inset-0 z-50 bg-black/40 flex items-start sm:items-center justify-center px-4 py-8 overflow-y-auto">
            <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden">
              <div className="flex items-center gap-3 px-6 py-5 bg-[#0F172A]">
                <div className={`w-11 h-11 rounded-full flex items-center justify-center text-white text-sm font-extrabold flex-shrink-0 ${avatarBg(payModal.customerId || payModal.name)}`}>
                  {initials(payModal.name)}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-white font-extrabold text-sm truncate">{payModal.name}</p>
                  <p className="text-white/50 text-[11px]">Due Amount</p>
                </div>
                <p className="text-red-400 font-extrabold text-sm whitespace-nowrap">{fmt(payModal.totalDue)}</p>
              </div>
              ...
              <div className="px-6 py-5">
                ...
                <div className="flex gap-4 mb-3 text-sm font-semibold text-[#334155]">
                  {["cash", "mobile", "bank"].map((m) => (
                    <label key={m} className="flex items-center gap-1.5 cursor-pointer">
                      <input type="radio" checked={payMethod === m} onChange={() => setPayMethod(m)} className="accent-[#16A34A]" />
                      {m === "mobile" ? "Mobile Banking" : m === "bank" ? "Bank" : "Cash"}
                    </label>
                  ))}
                </div>
                ...
                {payMethod === "mobile" && (
                  <div className="grid grid-cols-4 gap-2 mb-4">
                    {MOBILE_PROVIDERS.map((p) => (
                      <button key={p} onClick={() => setPayProvider(p)}
                        className={`flex flex-col items-center gap-1.5 py-3 rounded-xl border-2 transition ${
                          payProvider === p ? "border-[#16A34A] bg-emerald-50" : "border-[#E2E8F0] bg-white"
                        }`}>
                        <span className={`w-8 h-8 rounded-full flex items-center justify-center text-white ${MOBILE_ICON_BG[p] || "bg-gray-400"}`}>
                          <FiSmartphone size={14} />
                        </span>
                        <span className="text-[10px] font-bold text-[#334155]">{p}</span>
                      </button>
                    ))}
                  </div>
                )}
                ...
                {payMethod === "bank" && (
                  <div className="mb-4">
                    <div className="relative">
                      <FiCreditCard size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#94A3B8]" />
                ...
                <div className="flex gap-3 mt-2">
                  <button onClick={() => setPayModal(null)} className="flex-1 border border-[#E2E8F0] text-[#334155] font-bold py-3 rounded-xl hover:bg-[#F8FAFC] transition">Cancel</button>
                  <button onClick={submitPayment} disabled={payLoading} className="flex-1 bg-[#16A34A] hover:bg-[#15803D] text-white font-bold py-3 rounded-xl disabled:opacity-50 shadow-[0_4px_14px_-2px_rgba(22,163,74,0.35)] transition">
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-5">
          ...
          <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl sm:text-[28px] font-extrabold text-[#0F172A] tracking-tight">Due Customers</h1>
              <p className="text-[#94A3B8] text-sm mt-1">Manage due customers for your account</p>
            </div>
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex items-center gap-3 bg-white border border-[#F1F5F9] rounded-2xl px-4 py-3 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]">
                ...
                <div>
                  <p className="text-[10px] font-bold text-[#94A3B8] uppercase">Total Due Customers</p>
                  <p className="text-lg font-extrabold text-[#0F172A] leading-tight">{data.length}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 bg-white border border-[#F1F5F9] rounded-2xl px-4 py-3 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]">
                ...
                <div>
                  <p className="text-[10px] font-bold text-[#94A3B8] uppercase">Total Due Amount</p>
                  <p className="text-lg font-extrabold text-[#0F172A] leading-tight">{fmt(totalDueAmount)}</p>
                </div>
              </div>
            </div>
          </div>
              ...
              {[{ key: "all", label: "All Types" }, { key: "1", label: "Retail" }, { key: "2", label: "Wholesale" }].map(({ key, label }) => (
                <button key={key} onClick={() => setTypeFilter(key)}
                  className={`px-4 py-2 rounded-full text-sm font-bold transition border ${
                    typeFilter === key ? "bg-[#0F172A] text-white border-[#0F172A]" : "bg-white border-[#E2E8F0] text-[#64748B] hover:border-red-300"
                  }`}>
                  {label}
                </button>
              ))}
          ...
          <div className="bg-white border border-[#F1F5F9] rounded-2xl overflow-hidden shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]">
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-[#F8FAFC] border-b border-[#F1F5F9]">
                    <th className="px-5 py-3.5 w-10">
                      <input type="checkbox" checked={rows.length > 0 && selected.length === rows.length} onChange={toggleSelectAll} className="accent-[#16A34A] w-4 h-4 rounded" />
                    </th>
                    {["Customer", "Type", "Phone", "Address", "Orders", "Total Spent", "Due Amount", "Status", "Actions"].map((h) => (
                ...
                <tbody className="divide-y divide-[#F1F5F9]">
                  {filteredRows.length === 0 ? (
                    <tr><td colSpan={10} className="text-center py-16 text-[#94A3B8] text-sm">No due customers found 🎉</td></tr>
                  ) : rows.map((c) => {
                    const id = c.customerId || c.name;
                    return (
                      <tr key={id} className="hover:bg-[#F8FAFC] transition">
                        <td className="px-5 py-4">
                          <input type="checkbox" checked={selected.includes(id)} onChange={() => toggleSelect(id)} className="accent-[#16A34A] w-4 h-4 rounded" />
                        </td>
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0 ${avatarBg(id)}`}>{initials(c.name)}</div>
                            <div>
                              <div className="text-sm font-bold text-[#0F172A] whitespace-nowrap">{c.name}</div>
                              <div className="text-[11px] text-[#94A3B8]">Realistic Do.</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-4 text-sm text-[#64748B]"><span className="truncate block max-w-[150px]">{c.address || "—"}</span></td>
                        <td className="px-5 py-4 text-sm font-bold text-[#334155]">{c.invoiceCount}</td>
                        <td className="px-5 py-4 text-sm font-bold text-[#334155] whitespace-nowrap">{fmt(c.totalSpent)}</td>
                        <td className="px-5 py-4"><span className="bg-red-500 text-white text-xs font-bold px-2.5 py-1 rounded-full whitespace-nowrap">{fmt(c.totalDue)}</span></td>
                        <td className="px-5 py-4"><span className={`text-[11px] font-bold px-2.5 py-1 rounded-full capitalize ${c.status === "active" ? "bg-amber-50 text-amber-600" : "bg-[#F1F5F9] text-[#94A3B8]"}`}>{c.status === "active" ? "Rated" : c.status}</span></td>
                        <td className="px-5 py-4">
                          <button onClick={() => { setPayModal(c); setPayAmount(""); }}
            ...
            <div className="md:hidden divide-y divide-[#F1F5F9]">
              {filteredRows.length === 0 ? (
                <div className="text-center py-16 text-[#94A3B8] text-sm">No due customers 🎉</div>
              ) : rows.map((c) => {
                const id = c.customerId || c.name;
                return (
                  <div key={id} className="p-5">
                    <div className="flex items-start justify-between gap-2 mb-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-11 h-11 rounded-full flex items-center justify-center text-white font-bold text-sm ${avatarBg(id)}`}>{initials(c.name)}</div>
                        <div>
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-red-100 text-red-600">Due</span>
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-4">
                      ...
                      <div className="bg-emerald-50 rounded-xl p-3 text-center"><div className="text-xs font-bold text-emerald-700">{fmt(c.totalSpent)}</div><div className="text-[10px] text-[#94A3B8] mt-0.5 font-medium">Spent</div></div>
                      <div className="bg-red-50 rounded-xl p-3 text-center"><div className="text-xs font-bold text-red-600">{fmt(c.totalDue)}</div><div className="text-[10px] text-[#94A3B8] mt-0.5 font-medium">Due</div></div>
                    </div>
                    <div className="flex gap-2">
                      <a href={`tel:${c.phone}`} className="flex-1 flex items-center justify-center gap-1.5 bg-emerald-50 hover:bg-emerald-100 text-[#16A34A] font-bold py-2.5 rounded-xl text-xs transition"><FiPhone size={13} />Call</a>
                      <button onClick={() => setDrawer(c)} className="flex-1 flex items-center justify-center gap-1.5 bg-blue-50 hover:bg-blue-100 text-blue-600 font-bold py-2.5 rounded-xl text-xs transition"><FiEye size={13} />View</button>
                      <button onClick={() => { setPayModal(c); setPayAmount(""); }} className="flex-1 flex items-center justify-center gap-1.5 bg-[#0F172A] hover:bg-[#1E293B] text-white font-bold py-2.5 rounded-xl text-xs transition"><FiDollarSign size={13} />Pay</button>
                      <button onClick={() => setDelId(id)} className="flex-1 flex items-center justify-center gap-1.5 bg-red-50 hover:bg-red-100 text-red-500 font-bold py-2.5 rounded-xl text-xs transition"><FiTrash2 size={13} />Del</button>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="flex items-center justify-center gap-1.5 px-5 py-4 border-t border-[#F1F5F9]">
              <button type="button" disabled={page === 1} onClick={() => setPage(1)}
                className="w-8 h-8 rounded-lg border border-[#E2E8F0] text-xs font-bold text-[#64748B] disabled:opacity-40 hover:bg-[#F8FAFC] flex items-center justify-center">«</button>
              <button type="button" disabled={page === 1} onClick={() => setPage((p) => Math.max(1, p - 1))}
                className="w-8 h-8 rounded-lg border border-[#E2E8F0] text-xs font-bold text-[#64748B] disabled:opacity-40 hover:bg-[#F8FAFC] flex items-center justify-center"><FiChevronLeft size={14} /></button>
              <span className="w-8 h-8 rounded-lg bg-[#0F172A] text-white text-xs font-bold flex items-center justify-center">{page}</span>
              <button type="button" disabled={page === totalPages} onClick={() => setPage((p) => Math.min(totalPages, p + 1))}
                className="w-8 h-8 rounded-lg border border-[#E2E8F0] text-xs font-bold text-[#64748B] disabled:opacity-40 hover:bg-[#F8FAFC] flex items-center justify-center"><FiChevronRight size={14} /></button>
              <button type="button" disabled={page === totalPages} onClick={() => setPage(totalPages)}
                className="w-8 h-8 rounded-lg border border-[#E2E8F0] text-xs font-bold text-[#64748B] disabled:opacity-40 hover:bg-[#F8FAFC] flex items-center justify-center">»</button>
            </div>
          </div>

          <p className="text-center text-[#94A3B8] text-sm pb-4">Khulna Hardware Mart · Due Customers</p>
        </div>
      </div>
    </div>
  );
