  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:5000/api/invoices", {
        params: { customerName: c.name, paymentStatus: "paid", limit: 100 },
      })
      .then((res) => {
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-sm bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
        ...
        <div className="px-6 py-6 flex items-center gap-4 border-b border-[#F1F5F9]">
          <div
            className={`w-16 h-16 rounded-full flex items-center justify-center text-white text-xl font-extrabold ${avatarBg(c.customerId || c.name)}`}
          >
            {initials(c.name)}
          </div>
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
              ...
              <span
                className={`inline-flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-full font-bold ${typeStyle(c.customerType)}`}
              >
                {typeLabel(c.customerType)}
              </span>
            </div>
        ...
        <div className="px-6 pt-5">
          <a
            href={`tel:${c.phone}`}
            className="w-full flex items-center justify-center gap-2 bg-[#16A34A] hover:bg-[#15803D] text-white font-bold py-3.5 rounded-xl text-sm shadow-[0_4px_14px_-2px_rgba(22,163,74,0.35)] transition"
          >
        ...
        <div className="px-6 py-5 space-y-4">
          {[
            ...
            <div key={label} className="flex items-center justify-between">
              ...
              <span
                className={`text-sm font-bold ${val === "Not provided" ? "text-[#94A3B8] italic" : "text-[#0F172A]"}`}
              >
                {val}
              </span>
            </div>
        ...
        <div className="px-6 pb-5 grid grid-cols-2 gap-3">
          <div className="bg-[#F8FAFC] border border-[#F1F5F9] rounded-xl p-4">
            <div className="text-[#0F172A] text-lg font-extrabold">
              {fmt(c.totalPaid)}
            </div>
            <div className="text-[#94A3B8] text-xs mt-0.5 font-semibold">
              Total Spent
            </div>
            <div className="text-[#CBD5E1] text-[11px] mt-2 font-medium">
              Total Orders
            </div>
          </div>
        ...
        <div className="px-6 py-5">
          <p className="text-sm font-extrabold text-[#0F172A] mb-3 flex items-center gap-1.5">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-[#F8FAFC] border-b border-[#F1F5F9]">
                    <th className="text-left px-3 py-2.5 text-[10px] font-bold text-[#94A3B8] uppercase">
                      Invoice
                    </th>
                    <th className="text-left px-3 py-2.5 text-[10px] font-bold text-[#94A3B8] uppercase">
                      Date
                ...
                <tbody className="divide-y divide-[#F1F5F9]">
                  {invoices.map((inv) => (
                    <tr key={inv._id}>
                      <td className="px-3 py-2.5 font-bold text-[#0F172A] whitespace-nowrap">
                        {inv.invoiceNumber}
                      </td>
...
export default function PaidCustomers() {
  ...
  const fetchData = () => {
    setLoading(true);
    axios
      .get("http://localhost:5000/api/customers/paid", { params: { search } })
  ...
  const filtered = useMemo(() => {
    let rows = data;
    if (typeFilter !== "all")
      rows = rows.filter((c) => String(c.customerType) === String(typeFilter));
    return rows;
  }, [data, typeFilter]);
        ...
        <div className="max-w-375 mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-5">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[#94A3B8] text-sm mt-0.5">
                  Customers with no outstanding due — the good ones!
                </p>
              </div>
            </div>
                  ...
                  {rows.length === 0 ? (
                    ...
                    rows.map((c) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-4">
                          <span
                            className={`text-[11px] font-bold px-2.5 py-1 rounded-full capitalize ${c.status === "active" ? "bg-emerald-50 text-[#16A34A]" : "bg-[#F1F5F9] text-[#94A3B8]"}`}
                          >
                            {c.status}
                          </span>
                        </td>
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-1.5">
                            <a
                              href={`tel:${c.phone}`}
                              className="text-[11px] font-bold text-white bg-[#16A34A] hover:bg-[#15803D] px-3 py-1.5 rounded-full transition"
                            >
                              Call
                            </a>
                            <button
                              onClick={() => setDrawer(c)}
                              className="text-[11px] font-bold text-white bg-blue-600 hover:bg-blue-700 px-3 py-1.5 rounded-full transition"
                            >
                              View
                            </button>
                            <button
                              onClick={() => setDelId(c.customerId || c.name)}
                              className="text-[11px] font-bold text-white bg-red-500 hover:bg-red-600 px-3 py-1.5 rounded-full transition"
                            >
                              Delete
                            </button>
                          </div>
                ...
                rows.map((c) => (
                  <div key={c.customerId || c.name} className="p-5">
                    <div className="flex items-start justify-between gap-2 mb-4">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-11 h-11 rounded-full flex items-center justify-center text-white font-bold text-sm ${avatarBg(c.customerId || c.name)}`}
                        >
                          {initials(c.name)}
                        </div>
                        <div>
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-emerald-50 text-[#16A34A]">
                          Paid
                        </span>
                        <span
                    ...
                    <div className="flex gap-2">
                      <a
                        href={`tel:${c.phone}`}
                        className="flex-1 flex items-center justify-center gap-1.5 bg-[#16A34A] hover:bg-[#15803D] text-white font-bold py-2.5 rounded-xl text-xs transition"
                      >
                        <FiPhone size={13} />
                        Call
                      </a>
                      <button
                        onClick={() => setDrawer(c)}
                        className="flex-1 flex items-center justify-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 rounded-xl text-xs transition"
                      >
                        <FiEye size={13} />
                        View
                      </button>
                      <button
                        onClick={() => setDelId(c.customerId || c.name)}
                        className="flex-1 flex items-center justify-center gap-1.5 bg-red-500 hover:bg-red-600 text-white font-bold py-2.5 rounded-xl text-xs transition"
                      >
                        <FiTrash2 size={13} />
                        Remove
                      </button>
          ...
          <div className="bg-[#0F172A] rounded-2xl p-5">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-white/10 hover:bg-white/15 text-white font-bold px-6 py-3 rounded-xl text-sm transition"
            >
          ...
          <p className="text-center text-[#94A3B8] text-sm pb-4">
            Khulna Hardware Mart · Paid Customers
          </p>
        </div>
      </div>
    </div>
