...
import {
  FiAlertCircle, FiSearch, FiCalendar, FiUser, FiPhone,
...
export default function DueInvoice() {
  ...
  const printInvoice = (inv) => {
    ...
    w.document.write(`
      ...
      <p style="text-align:right;margin-top:12px;">Grand Total: ${fmt(inv.grandTotal)}<br/>
      Paid: ${fmt(inv.paidAmount)}<br/>
      <strong style="font-size:18px;color:#ef4444;">Due: ${fmt(inv.dueAmount)}</strong></p>
      <p style="color:#64748b;font-size:13px;">Payment so far: ${payments}</p>
      </body></html>
    `);
    w.document.close();
    w.focus();
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="w-full space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Due Invoices</h1>
            <p className="text-gray-500">{total.toLocaleString()} invoices with outstanding balance</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 flex flex-wrap items-center gap-3">
          ...
          <div className="bg-red-50 text-red-600 font-bold px-4 py-2 rounded-xl text-sm">
            Page Due Total: {fmt(totalDueSum)}
          </div>
        </div>
