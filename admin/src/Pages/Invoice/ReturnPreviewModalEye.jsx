      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 print:hidden">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => openPrintWindow(buildReturnHTML(invoice))} disabled={!invoice} className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-semibold disabled:opacity-50"><FiPrinter size={14} /> Print</button>
            <button
              onClick={onClose}
        ...
        <div id="return-print" className="p-6">
          {loading && (
            <div className="flex items-center justify-center gap-2 py-16 text-blue-600">
              <FiLoader className="animate-spin" size={20} /> Loading…
            </div>
          )}
          {error && (
            ...
            <>
              ...
              <div className="flex justify-end mb-4">
                <p className="text-lg font-bold text-gray-900">
                  Grand Total: {fmt(invoice.grandTotal)}
                </p>
              </div>
              ...
              {invoice.returnedItems?.length > 0 && (
                <div className="border-t-4 border-dashed border-red-200 pt-4">
                  ...
                  <table className="w-full text-sm border-collapse mb-3">
                    <thead>
                      <tr className="bg-red-50 border-b border-red-100 text-xs uppercase text-red-500">
                        <th className="text-left px-2 py-2">Product</th>
                        <th className="text-center px-2 py-2">Qty Returned</th>
                        <th className="text-right px-2 py-2">Return Amount</th>
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                    <p className="text-sm text-gray-700">
                      ...
                      <span className="font-bold text-red-600">
                        {fmt(invoice.totalReturnedAmount || 0)}
                      </span>
                      , Final Amount:{" "}
