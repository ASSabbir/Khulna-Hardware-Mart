...
const Invoice = () => {
  ...
  if (restoreParam) {
    try {
      incomingDraft = JSON.parse(
        sessionStorage.getItem("khm_restore_draft") || "null",
      );
    } catch {
      incomingDraft = null;
    }
  ...
  const initialDraft = incomingDraft
    ? {
        ...draft,
        ...incomingDraft,
        invoiceNum: incomingDraft.draftNum || incomingDraft.invoiceNum,
        invoiceDate: incomingDraft.draftDate || incomingDraft.invoiceDate,
      }
    : draft;
  ...
  const [customer, setCustomer] = useState(
    initialDraft.customer || { name: "", phone: "", address: "" },
  );
  const [invoiceNum, setInvoiceNum] = useState(
    initialDraft.invoiceNum || invoiceNo(),
  );
  const [invoiceDate, setInvoiceDate] = useState(
    initialDraft.invoiceDate || today(),
  );
  const [discount, setDiscount] = useState(initialDraft.discount || "");
  const [priceType, setPriceType] = useState(
    initialDraft.priceType || "retail",
  );
  const [toast, setToast] = useState(null);

  const [vatEnabled, setVatEnabled] = useState(
    initialDraft.vatEnabled || false,
  );
  const [customerType, setCustomerType] = useState(
    initialDraft.customerType || "retail",
  );
  const [transportEnabled, setTransportEnabled] = useState(
    initialDraft.transportEnabled || false,
  );
  const [transportCost, setTransportCost] = useState(
    initialDraft.transportCost || "",
  );
  const [preparedBy, setPreparedBy] = useState(
    initialDraft.preparedBy || PREPARED_BY_OPTIONS[0],
  );
  const [isPreparedByOther, setIsPreparedByOther] = useState(
    initialDraft.isPreparedByOther || false,
  );
  const [customPreparedBy, setCustomPreparedBy] = useState(
    initialDraft.customPreparedBy || "",
  );
  ...
  const [mobileProvider, setMobileProvider] = useState(
    initialDraft.mobileProvider || MOBILE_BANKING_PROVIDERS[0],
  );
  const [mobileNumber, setMobileNumber] = useState(
    initialDraft.mobileNumber || "",
  );
  const [bankName, setBankName] = useState(
    initialDraft.bankName || BANK_OPTIONS[0],
  );
  const [bankAccountNumber, setBankAccountNumber] = useState(
    initialDraft.bankAccountNumber || "",
  );

  const [paymentStatus, setPaymentStatus] = useState(
    initialDraft.paymentStatus || "paid",
  );
  const [paidNowAmount, setPaidNowAmount] = useState(
    initialDraft.paidNowAmount || "",
  );
  ...
  const [splitRows, setSplitRows] = useState(
    Array.isArray(initialDraft.splitRows) && initialDraft.splitRows.length > 0
      ? initialDraft.splitRows
      : [emptySplitRow()],
  );
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:5000/api/customers?search=${encodeURIComponent(phone)}`,
      )
      .then((res) => {
        if (cancelled) return;
  ...
  const addCustomProductToMemo = async () => {
    ...
    if (!name) {
      showToast("error", "Enter a product name first.");
      return;
    }
    if (unitPrice <= 0) {
      showToast("error", "Enter a valid unit price.");
      return;
    }
    ...
    try {
      const res = await axios.post(
        "http://localhost:5000/api/products/custom-quick",
        { name, unitPrice, qty, shopName: customProduct.shopName.trim() },
      );
      ...
      setMemoItems((prev) => {
        ...
        return [
          ...prev,
          {
            ...
            unit: "pcs",
            price: unitPrice,
            retailPriceValue: unitPrice,
            holcellPriceValue: unitPrice,
            qty,
            stock: product.stock,
            suppliers: [],
            preferredSupplierId: "",
  ...
  const transportAmt = transportEnabled
    ? Math.max(0, parseFloat(transportCost) || 0)
    : 0;
  ...
  const handleCompleteSale = async () => {
    ...
    if (isPreparedByOther ? !customPreparedBy.trim() : !preparedBy) {
      showToast("error", "Prepared By is required.");
      return;
    }
      ...
      try {
        ...
        try {
          const searchRes = await axios.get(
            `http://localhost:5000/api/customers?search=${encodeURIComponent(customer.name)}`,
          );
          const existingCustomer = searchRes.data.customers.find(
            (c) => c.name.toLowerCase() === customer.name.toLowerCase(),
          );
          if (existingCustomer)
            await axios.put(
              `http://localhost:5000/api/customers/${existingCustomer._id}`,
              {
  ...
  const printChallan = () => {
    if (memoItems.length === 0) {
      showToast("error", "Add at least one product to print a challan.");
      return;
    }
    openPrintWindow(
      buildChallanHTML({ invoiceNum, customer, items: memoItems }),
    );
  };
  ...
  const saveCurrentAsDraft = () => {
    if (memoItems.length === 0) {
      showToast("error", "Add at least one product before saving a draft.");
      return;
    }
    upsertDraft({
      id: invoiceNum,
      draftNum: invoiceNum,
      draftDate: invoiceDate,
      memoItems,
      customer,
      discount,
      priceType,
  ...
  const loadSavedDraft = (d) => {
    setMemoItems(d.memoItems || []);
    setCustomer(d.customer || { name: "", phone: "", address: "" });
    setDiscount(d.discount || "");
    setPriceType(d.priceType || "retail");
    setInvoiceNum(d.draftNum || invoiceNo());
  ...
  return (
    <>
      <link
        href="https://fonts.googleapis.com/css2?family=Barlow:wght@400;500;600;700&family=Barlow_Condensed:wght@600;700&display=swap"
        rel="stylesheet"
      />
      <style>{`
        @media print {
      ...
      {savedDrafts.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4 font-['Barlow',sans-serif]">
          {savedDrafts.map((d) => (
            <div
              key={d.id}
              className="flex items-center gap-2 bg-white border-2 border-slate-200 rounded-lg px-3 py-2 text-xs"
            >
              <button
                onClick={() => loadSavedDraft(d)}
                className="font-bold text-[#1E3A8A] hover:underline"
              >
                {d.draftNum} · {d.memoItems.length} items ·{" "}
        ...
        <div className="xl:w-120 shrink-0 flex flex-col gap-4 xl:sticky xl:top-4 xl:self-start xl:max-h-[calc(100vh-32px)]">
            ...
            {!loading &&
              ...
              products.map((p) => {
                ...
                return (
                  <div
                    key={p._id}
                    className={`group flex items-center gap-3 bg-white border-2 rounded-xl px-4 py-3 transition-all duration-150 ${inMemo ? "border-[#F97316] bg-[#FFF7ED]" : "border-slate-200 hover:border-[#1D4ED8]"}`}
                  >
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-[#1E293B] truncate">
                        {p.name}
                        {p.isCustom && (
                          <span className="ml-1.5 text-[9px] font-bold text-amber-600 bg-amber-50 border border-amber-200 px-1 py-0.5 rounded">
                            CUSTOM
                          </span>
                        )}
                      </p>
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        <span className="text-[10px] font-semibold bg-[#EFF6FF] text-[#1D4ED8] px-1.5 py-0.5 rounded border border-[#BFDBFE]">
                          {p.brand || p.company || "—"}
                        </span>
                    ...
                    <button
                      ...
                      className={`shrink-0 w-8 h-8 rounded-lg border-2 flex items-center justify-center transition-colors ${inMemo || p.stock === 0 ? "border-[#F97316] bg-[#F97316] text-white cursor-default" : "border-slate-200 text-slate-400 hover:border-[#1D4ED8] hover:text-[#1D4ED8]"}`}
                    >
                      <FiPlus size={14} />
                    </button>
                  </div>
          ...
          <div className="bg-white border-2 border-dashed border-slate-300 rounded-xl p-4 flex flex-col gap-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-['Barlow_Condensed',sans-serif] font-bold text-[#1E3A8A] text-sm uppercase tracking-wide">
                Add Custom Product
              </h3>
            </div>
            <input
            ...
            <div className="grid grid-cols-2 gap-2">
              <input
                ...
                onChange={(e) =>
                  setCustomProduct((c) => ({ ...c, qty: e.target.value }))
                }
                className="w-full text-sm border-2 border-slate-200 rounded-lg px-3 py-2 outline-none focus:border-[#1D4ED8] transition-colors font-['Barlow',sans-serif]"
              />
              <div className="flex items-center border-2 border-slate-200 rounded-lg overflow-hidden focus-within:border-[#1D4ED8] transition-colors">
                <span className="px-2 text-xs text-slate-400 bg-slate-50 border-r border-slate-200 h-full flex items-center">
                  ৳
                </span>
                <input
                  type="number"
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 bg-[#1E3A8A] hover:bg-[#1D4ED8] text-white text-xs font-bold py-2.5 rounded-lg transition-colors disabled:opacity-60"
            >
              {addingCustom ? (
        ...
        <div className="flex-1 flex flex-col gap-4 min-w-0">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-medium">
                  {memoItems.length} item{memoItems.length !== 1 ? "s" : ""} ·{" "}
                  {fmt(grandTotal)}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-1.5 border-2 border-[#1D4ED8] rounded-lg text-[#1D4ED8] text-xs font-semibold hover:bg-blue-50 transition-colors"
              >
                Save Draft
              </button>
              <button
                onClick={clearMemo}
                className="px-3 py-1.5 border-2 border-slate-200 rounded-lg text-slate-500 text-xs font-semibold hover:border-slate-300 transition-colors"
              >
                Clear
              </button>
            </div>
          ...
          >
            <div className="hidden print:flex flex-col items-center gap-2 pt-2 pb-4 border-b-2 border-slate-100">
              ...
              <p className="text-slate-400 text-[10px] font-medium text-center max-w-md">
                ...
                02477-721990, +880 1931-272839, +880 1679-123205
              </p>
            </div>
            ...
            <div className="bg-[#1E3A8A] px-6 py-4 flex items-center justify-between flex-wrap gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[#93C5FD] text-[10px] font-medium">
                    280-Khanjahan Ali Road (Rahmania Madrasha Complex), Khulna·
                    02477-721990 , +880 1931-272839 , +880 1679-123205{" "}
                  </p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 px-5 py-4 border-b-2 border-slate-100 bg-slate-50">
              {[
                ...
                },
                {
                  ...
                  key: "phone",
                  placeholder: "Phone Number (auto-fills name)",
                },
                {
            ...
            <div className="flex flex-wrap items-center gap-3 px-5 py-3 border-b-2 border-slate-100 bg-white print:hidden">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg border-2 text-xs font-bold transition-colors ${customerType === "retail" ? "border-[#F97316] bg-[#F97316] text-white" : "border-slate-200 text-slate-500"}`}
                >
                  Retail
                </button>
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg border-2 text-xs font-bold transition-colors ${customerType === "wholesale" ? "border-[#1D4ED8] bg-[#1D4ED8] text-white" : "border-slate-200 text-slate-500"}`}
                >
                  Wholesale
                </button>
              </div>
              <span className="text-[10px] text-slate-400">
                Switching updates item prices automatically. Not printed on
                invoice.
              </span>
            </div>

            <div className="flex flex-wrap items-center gap-3 px-5 py-3 border-b-2 border-slate-100 bg-slate-50">
              <span className="text-xs font-bold text-[#1E3A8A] uppercase tracking-wider print:hidden">
                Prepared By <span className="text-red-500">*</span>
              </span>
              <div className="flex items-center gap-2 print:hidden">
                <select
                  value={isPreparedByOther ? "__others__" : preparedBy}
                  onChange={(e) => {
                    const val = e.target.value;
                    if (val === "__others__") setIsPreparedByOther(true);
                    else {
                      setIsPreparedByOther(false);
                      setPreparedBy(val);
                    }
                  }}
                  className="text-xs font-semibold border-2 border-slate-200 rounded-lg px-3 py-2 outline-none bg-white"
                >
                  {PREPARED_BY_OPTIONS.map((n) => (
              ...
              <span className="text-xs font-semibold text-slate-700">
                Prepared by:{" "}
                {isPreparedByOther ? customPreparedBy || "—" : preparedBy}
              </span>
            </div>
            ...
            <div className="flex-1">
              {memoItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 gap-3 text-slate-300">
                  ...
                  <p className="text-xs">
                    Search products on the left and click <strong>+</strong> to
                    add
                  </p>
                </div>
              ) : (
                      ...
                      >
                        ...
                        <td className="px-4 py-2.5 text-right">
                          <div className="flex items-center justify-end border-2 border-slate-200 rounded-lg overflow-hidden focus-within:border-[#1D4ED8] transition-colors">
                            <span className="px-2 py-1 text-xs text-slate-400 bg-slate-50 border-r border-slate-200">
                              ৳
                            </span>
                            <input
                              type="number"
              ...
              <div className="border-t-2 border-slate-100 px-5 py-4 bg-slate-50">
                <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                  <div className="flex flex-col gap-3">
                    <div className="flex items-center gap-3">
                      <label className="text-xs font-bold text-[#1E3A8A] uppercase tracking-wider whitespace-nowrap">
                        Discount (৳)
                      </label>
                      <div className="flex items-center border-2 border-slate-200 rounded-lg overflow-hidden focus-within:border-[#F97316] transition-colors">
                        <span className="px-2 py-2 text-xs text-slate-400 bg-white border-r border-slate-200">
                          ৳
                        </span>
                        <input
                          type="number"
                          ...
                          value={discount}
                          onChange={(e) =>
                            setDiscount(clampToMax(e.target.value, subtotal))
                          }
                    ...
                    <div className="flex items-center gap-3 print:hidden">
                      <label className="flex items-center gap-2 text-xs font-bold text-[#1E3A8A] uppercase tracking-wider cursor-pointer select-none">
                        <input
                          ...
                          onChange={(e) =>
                            setTransportEnabled(e.target.checked)
                          }
                          className="w-4 h-4 accent-[#1E3A8A]"
                        />{" "}
                        Transport Cost
                  ...
                  <div className="flex flex-col items-end gap-1 min-w-[200px]">
                    ...
                    {transportEnabled && transportAmt > 0 && (
                      <div className="flex justify-between w-full text-xs text-slate-500 font-semibold">
                    ...
                    <div className="flex justify-between w-full pt-1.5 border-t-2 border-[#1E3A8A] mt-1">
                      <span className="font-['Barlow_Condensed',sans-serif] font-bold text-[#1E3A8A] uppercase tracking-wide text-sm">
                        Grand Total
                      </span>
                      <span className="font-['Barlow_Condensed',sans-serif] font-bold text-[#F97316] text-lg tabular-nums leading-tight">
                        {fmt(grandTotal)}
                      </span>
                    </div>
                  </div>
                ...
                <div className="mt-4 pt-4 border-t border-slate-200 print:hidden">
                  ...
                  {paymentStatus === "due" && (
                    <div className="flex items-center gap-3 bg-red-50 border-2 border-red-200 rounded-lg px-3 py-2">
                      <label className="text-xs font-bold text-red-600 whitespace-nowrap">
                        Paying Now (৳)
                      </label>
                      <input
                        type="number"
                  ...
                  {!splitPayment ? (
                    ...
                    <div className="flex flex-col gap-2">
                      {splitRows.map((row) => (
                        <div
                          key={row.id}
                          className="flex flex-wrap items-center gap-2 bg-slate-50 border-2 border-slate-200 rounded-lg p-2"
                        >
                          <select
                            value={row.method}
                            onChange={(e) =>
                          ...
                          <div className="flex items-center border-2 border-slate-200 rounded-lg overflow-hidden bg-white">
                            ...
                            <input
                              ...
                              placeholder="0.00"
                              className="w-24 px-1 py-1.5 text-xs font-semibold outline-none font-['Barlow',sans-serif]"
                            />
                          </div>
                          {row.method === "mobile" && (
                            <>
                      ...
                      <div
                        className={`flex items-center justify-between text-xs font-bold px-1 ${splitMismatch ? "text-red-600" : "text-green-600"}`}
                      >
                        <span>Split Total: ৳{splitTotal.toFixed(2)}</span>
                        <span>
                          Remaining: ৳
                          {Math.max(0, expectedPaidAmount - splitTotal).toFixed(
                            2,
                          )}
                        </span>
                      </div>
                ...
                <div className="hidden print:block mt-4 pt-4 border-t border-slate-200">
                  <p className="text-xs font-bold text-[#1E3A8A] uppercase tracking-wider mb-2">
                    Payment Details
                  </p>
                  <div className="space-y-1">
                    {buildPayments().map((p, i) => (
                      ...
                      >
                        <span>
                          {p.method === "mobile"
                            ? `Mobile Banking (${p.provider}${p.mobileNumber ? " - " + p.mobileNumber : ""})`
                            : p.method === "bank"
                              ? `Bank (${p.bankName}${p.accountNumber ? " - " + p.accountNumber : ""})`
                              : "Cash"}
                        </span>
                    ...
                    <div className="flex justify-between text-sm font-bold text-slate-900 pt-1 border-t border-slate-100">
                      ...
                      <span>{fmt(expectedPaidAmount)}</span>
                    </div>
                    {paymentStatus === "due" && dueBalance > 0 && (
                ...
                <p className="text-center text-[10px] text-slate-300 font-medium mt-3 tracking-widest uppercase">
                  Thank you for shopping at Khulna Hardware Mart · Centenary
                  Est. 1924
                </p>
              </div>
            )}
          ...
          <div className="flex items-center justify-end gap-2 print:hidden">
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 bg-green-600 border-2 border-green-600 text-white rounded-lg text-sm font-bold hover:bg-green-700 hover:border-green-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {saving ? (
            ...
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 bg-slate-600 border-2 border-slate-600 text-white rounded-lg text-sm font-bold hover:bg-slate-700 hover:border-slate-700 transition-colors"
            >
              <FiFileText size={16} /> Challan Print
            </button>
            <button
              onClick={handlePrint}
              className="flex items-center gap-2 px-6 py-3 bg-[#1E3A8A] border-2 border-[#1E3A8A] text-white rounded-lg text-sm font-bold hover:bg-[#1D4ED8] hover:border-[#1D4ED8] transition-colors"
            >
