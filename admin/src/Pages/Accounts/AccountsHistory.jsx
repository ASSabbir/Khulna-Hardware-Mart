...
function DonutChart({ income, expense, size = 120, stroke = 16 }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="-rotate-90 mx-auto">
      <circle cx={size / 2} cy={size / 2} r={r} stroke="#EF4444" strokeWidth={stroke} fill="none" />
      <circle cx={size / 2} cy={size / 2} r={r} stroke="#3B82F6" strokeWidth={stroke} fill="none"
        strokeDasharray={`${incomeLen} ${c - incomeLen}`} strokeLinecap="round" />
    </svg>
  );
}
  ...
  return (
    <div className="relative mx-auto" style={{ width: size, height: size / 2 + 20 }}>
      <svg width={size} height={size / 2 + 10} viewBox={`0 0 ${size} ${size / 2 + 10}`}>
        ...
        <path d={`M 12 ${size / 2} A ${r} ${r} 0 0 1 ${size - 12} ${size / 2}`} stroke="#F1F5F9" strokeWidth="12" fill="none" strokeLinecap="round" />
        <path d={`M 12 ${size / 2} A ${r} ${r} 0 0 1 ${size - 12} ${size / 2}`} stroke="url(#gaugeGrad)" strokeWidth="12" fill="none"
...
export default function AccountsHistory() {
  ...
  const healthScore = useMemo(() => {
    ...
    if (!total) return 0;
    return Math.max(0, Math.min(100, Math.round((totals.income / total) * 100)));
  }, [totals]);
  ...
  const exportCSV = () => {
    const rows = [["Date", "Type", "Category", "Method", "Description", "Amount"]];
    filteredSorted.forEach((t) => rows.push([fmtDate(t.date), t.type, t.category, t.method, t.description, t.amount]));
    ...
    <div className="min-h-screen bg-[#F8FAFC] font-sans">
      <div className="max-w-[1700px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="flex items-center gap-2 text-sm text-[#94A3B8] font-medium mb-4">
          <FiHome size={13} /> Home <FiChevronRight size={12} /> ERP <FiChevronRight size={12} />
          <span className="text-[#0F172A] font-bold">Money &amp; Expense History</span>
        </div>

        {/* Header */}
        <div className="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-4 mb-6 sm:mb-8">
          <div>
            <h1 className="text-2xl sm:text-[28px] font-extrabold text-[#0F172A] tracking-tight">Money &amp; Expense History</h1>
            <p className="text-[#94A3B8] text-sm mt-1">Complete financial transaction records and analytics</p>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
            <button type="button" onClick={() => { setFrom(""); setTo(""); }}
              ...
              <FiCalendar size={15} className="text-[#64748B]" /> {from ? fmtDate(from) : "Current " + new Date().toLocaleDateString("en-US", { month: "long", year: "numeric" })}
            </button>
        ...
        <div className="grid grid-cols-1 2xl:grid-cols-[1fr_340px] gap-5 sm:gap-6">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
              <div className="bg-[#ECFDF5] border border-emerald-100 rounded-2xl p-5">
                <div className="flex items-center justify-between">
                  <p className="text-[#065F46] text-sm font-bold">Total Income</p>
                  <KpiIcon icon={FiTrendingUp} bg="bg-white/70" color="text-emerald-600" />
                </div>
                <p className="text-2xl font-extrabold text-[#0F172A] mt-2">{fmt(totals.income)}</p>
                <p className="text-emerald-600 text-xs font-bold mt-1 flex items-center gap-1"><FiArrowUpRight size={12} /> 12.5%</p>
              </div>
              <div className="bg-[#FEF2F2] border border-red-100 rounded-2xl p-5">
                <div className="flex items-center justify-between">
                  <p className="text-[#991B1B] text-sm font-bold">Total Expense</p>
                  <KpiIcon icon={FiTrendingDown} bg="bg-white/70" color="text-red-500" />
                </div>
                <p className="text-2xl font-extrabold text-[#0F172A] mt-2">{fmt(totals.expense)}</p>
                <p className="text-red-500 text-xs font-bold mt-1 flex items-center gap-1"><FiArrowDownRight size={12} /> 3.1%</p>
              </div>
              <div className="bg-white border border-[#F1F5F9] rounded-2xl p-5 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]">
                <div className="flex items-center justify-between">
                  <p className="text-[#64748B] text-sm font-bold">Net Cash Flow</p>
                  <KpiIcon icon={FiActivity} bg="bg-blue-50" color="text-blue-600" />
                </div>
                <p className={`text-2xl font-extrabold mt-2 ${totals.net >= 0 ? "text-[#10B981]" : "text-[#EF4444]"}`}>
                  {totals.net >= 0 ? "+" : ""}{fmt(totals.net)}
                </p>
                <p className="text-[#94A3B8] text-xs mt-1">Dynamic color state</p>
              </div>
              <div className="bg-white border border-[#F1F5F9] rounded-2xl p-5 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]">
                <div className="flex items-center justify-between">
                  <p className="text-[#64748B] text-sm font-bold">Transaction Count</p>
                  <KpiIcon icon={FiZap} bg="bg-[#F1F5F9]" color="text-[#0F172A]" />
                </div>
                <p className="text-2xl font-extrabold text-[#0F172A] mt-2">{txns.length.toLocaleString()}</p>
            ...
            <div className="bg-white border border-[#F1F5F9] rounded-2xl p-5 sm:p-6 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]">
              <h2 className="text-base font-extrabold text-[#0F172A] mb-4">Smart Filter Center</h2>

              <div className="flex flex-wrap gap-2 mb-5">
                {QUICK_RANGES.map((r) => (
                  <button key={r.label}
                    onClick={() => { setFrom(r.from); setTo(""); setActiveQuick(r.label); setPage(1); }}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                <div>
                  <label className="text-[11px] font-bold text-[#94A3B8] uppercase tracking-wide">Date range</label>
                  <div className="flex items-center gap-2 mt-1.5">
                    <FiCalendar className="text-[#94A3B8]" size={15} />
                    <input type="date" value={from} onChange={(e) => { setFrom(e.target.value); setActiveQuick(""); setPage(1); }}
                      className="border border-[#E2E8F0] rounded-lg px-3 py-2 text-sm w-full bg-[#F8FAFC]" />
                  </div>
                </div>
                <div>
                  <label className="text-[11px] font-bold text-[#94A3B8] uppercase tracking-wide">To</label>
                  <input type="date" value={to} onChange={(e) => { setTo(e.target.value); setActiveQuick(""); setPage(1); }}
                    className="border border-[#E2E8F0] rounded-lg px-3 py-2 text-sm w-full bg-[#F8FAFC] mt-1.5" />
                </div>
                <div>
                  <label className="text-[11px] font-bold text-[#94A3B8] uppercase tracking-wide">Transaction Type</label>
                  <div className="flex items-center gap-2 mt-1.5">
                ...
                <div>
                  <label className="text-[11px] font-bold text-[#94A3B8] uppercase tracking-wide">Search</label>
                  <div className="relative mt-1.5">
                    <FiSearch size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#94A3B8]" />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-4">
              <div className="bg-white border border-[#F1F5F9] rounded-2xl p-4 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]">
                <p className="text-[11px] font-bold text-[#94A3B8] uppercase">Income vs Expense</p>
                <div className="h-1.5 bg-[#FEF2F2] rounded-full overflow-hidden mt-2">
                  <div className="h-full bg-[#10B981] rounded-full" style={{ width: `${(totals.income / (totals.income + totals.expense || 1)) * 100}%` }} />
                </div>
                <p className="text-xs font-bold text-[#0F172A] mt-1.5">
                  {Math.round((totals.income / (totals.income + totals.expense || 1)) * 100)}% ratio
                </p>
              </div>
              <div className="bg-white border border-[#F1F5F9] rounded-2xl p-4 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]">
                <p className="text-[11px] font-bold text-[#94A3B8] uppercase">Cash Flow</p>
                <p className={`text-sm font-extrabold mt-2 ${totals.net >= 0 ? "text-[#10B981]" : "text-[#EF4444]"}`}>
                  {totals.net >= 0 ? "+" : ""}{fmt(totals.net)}
                </p>
              </div>
              <div className="bg-white border border-[#F1F5F9] rounded-2xl p-4 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]">
                <p className="text-[11px] font-bold text-[#94A3B8] uppercase">Highest Transaction</p>
                <p className="text-sm font-extrabold text-[#0F172A] mt-2">{fmt(highestTxn)}</p>
              </div>
              <div className="bg-white border border-[#F1F5F9] rounded-2xl p-4 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]">
                <p className="text-[11px] font-bold text-[#94A3B8] uppercase">Most Active Category</p>
                <p className="text-sm font-extrabold text-[#0F172A] mt-2 truncate">{topCategory}</p>
              </div>
              <div className="bg-[#EFF6FF] border border-blue-100 rounded-2xl p-4 flex items-center justify-between gap-2">
                <div>
                  <p className="text-[11px] font-bold text-blue-500 uppercase">Daily Average</p>
                  <p className="text-sm font-extrabold text-[#0F172A] mt-2">{fmt(Math.round(dailyAvg))}</p>
                </div>
                <FiChevronRight size={16} className="text-blue-400 flex-shrink-0" />
              </div>
            </div>
            ...
            <div className="bg-white border border-[#F1F5F9] rounded-2xl overflow-hidden shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]">
              <div className="px-5 sm:px-6 py-5 border-b border-[#F1F5F9] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <h2 className="text-base font-extrabold text-[#0F172A]">Transaction History</h2>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <button type="button" onClick={() => setSortDir((d) => (d === "desc" ? "asc" : "desc"))}
                    ...
                    {sortDir === "desc" ? <FiArrowDown size={13} /> : <FiArrowUp size={13} />} Sort
                  </button>
                  <button type="button"
                    className="flex items-center gap-1.5 text-xs font-bold px-3 py-2 rounded-full border border-[#E2E8F0] text-[#64748B] hover:bg-[#F8FAFC] transition">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-[#F8FAFC] border-b border-[#F1F5F9]">
                      {["Date", "Transaction Type", "Category", "Payment Method", "Description", "Amount"].map((h) => (
                  ...
                  <tbody className="divide-y divide-[#F1F5F9]">
                    {loading ? (
                      <tr><td colSpan={7} className="text-center py-16 text-[#94A3B8] text-sm">Loading...</td></tr>
                    ) : paged.length === 0 ? (
                      <tr><td colSpan={7} className="text-center py-16 text-[#94A3B8] text-sm">No entries found.</td></tr>
                    ) : paged.map((t) => (
                      <tr key={t.id} className="hover:bg-[#F8FAFC] transition">
                        <td className="px-5 py-4 text-sm text-[#64748B] whitespace-nowrap">{fmtDate(t.date)}</td>
                        ...
                        <td className="px-5 py-4">
                          <span className="text-[11px] font-semibold bg-[#F1F5F9] text-[#475569] px-2.5 py-1 rounded-full">{t.category}</span>
                        </td>
                        <td className="px-5 py-4">
                          <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full ${METHOD_STYLES[t.method] || "bg-[#F1F5F9] text-[#475569]"}`}>
                            {t.method}
                          </span>
                        </td>
              ...
              <div className="md:hidden divide-y divide-[#F1F5F9]">
                {loading ? (
                  <p className="text-center text-[#94A3B8] text-sm py-10">Loading...</p>
                ) : paged.length === 0 ? (
                  <p className="text-center text-[#94A3B8] text-sm py-10">No entries found.</p>
                ) : paged.map((t) => (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                          <span className="text-[11px] bg-[#F1F5F9] text-[#64748B] px-2 py-0.5 rounded-full font-medium">{t.category}</span>
                          <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${METHOD_STYLES[t.method] || "bg-[#F1F5F9] text-[#475569]"}`}>{t.method}</span>
              ...
              <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-t border-[#F1F5F9]">
                ...
                <div className="flex items-center gap-1.5">
                  <button type="button" disabled={page === 1} onClick={() => setPage(1)}
                    className="w-8 h-8 rounded-lg border border-[#E2E8F0] text-xs font-bold text-[#64748B] disabled:opacity-40 hover:bg-[#F8FAFC]">«</button>
          ...
          <div className="space-y-5">
            <h2 className="text-lg font-extrabold text-[#0F172A]">Premium Analytics</h2>

            <div className="bg-white border border-[#F1F5F9] rounded-2xl p-5 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]">
              <p className="text-sm font-bold text-[#0F172A]">Recent Financial Activity</p>
              ...
              <div className="flex items-center justify-center gap-4 mt-3 text-xs font-semibold text-[#64748B]">
                <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[#3B82F6]" /> Income</span>
                <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[#EF4444]" /> Expense</span>
              </div>
            </div>

            <div className="bg-white border border-[#F1F5F9] rounded-2xl p-5 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]">
              <p className="text-sm font-bold text-[#0F172A] mb-2">Financial Health Score</p>
              <HealthGauge value={healthScore} />
              <p className="text-center text-xl font-extrabold text-[#0F172A] -mt-1">{healthScore}</p>
            </div>

            <div className="bg-white border border-[#F1F5F9] rounded-2xl p-5 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]">
              <p className="text-sm font-bold text-[#0F172A] mb-3">Cash Flow Summary</p>
              <div className="space-y-2.5">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-[#64748B] font-medium">Income source</span>
                  <span className="font-bold text-[#0F172A]">{fmt(totals.income)}</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-[#64748B] font-medium">Expense</span>
                  <span className="font-bold text-[#EF4444]">-{fmt(totals.expense)}</span>
                </div>
                <div className="flex items-center justify-between text-xs pt-2 border-t border-[#F1F5F9]">
                  ...
                  <span className={`font-bold ${totals.net >= 0 ? "text-[#10B981]" : "text-[#EF4444]"}`}>
                    {totals.net >= 0 ? "+" : ""}{fmt(totals.net)}
                  </span>
                </div>
              </div>
            ...
            <div className="bg-white border border-[#F1F5F9] rounded-2xl p-5 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)]">
              <p className="text-sm font-bold text-[#0F172A] mb-3">Smart Insights</p>
              <div className="space-y-2.5">
                <div className="bg-[#ECFDF5] rounded-xl p-3 flex items-start gap-2">
                  <FiAward size={14} className="text-[#10B981] mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="text-xs font-bold text-[#065F46]">Highest income source</p>
                    <p className="text-[11px] text-emerald-700/70">{topCategory}</p>
                  </div>
                </div>
                <div className="bg-[#FEF2F2] rounded-xl p-3 flex items-start gap-2">
                  <FiTrendingDown size={14} className="text-[#EF4444] mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="text-xs font-bold text-[#991B1B]">Largest expense category</p>
                    <p className="text-[11px] text-red-700/70">{topCategory}</p>
                  </div>
                </div>
                <div className="bg-[#FFFBEB] rounded-xl p-3 flex items-start gap-2">
                  <FiAlertTriangle size={14} className="text-[#F59E0B] mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="text-xs font-bold text-[#92400E]">Spending warning</p>
                    <p className="text-[11px] text-amber-700/70">Should be monitored — professional review recommended</p>
                  </div>
                </div>
                <div className="bg-[#EFF6FF] rounded-xl p-3 flex items-start gap-2">
                  <FiZap size={14} className="text-[#3B82F6] mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="text-xs font-bold text-[#1D4ED8]">Revenue growth status</p>
                    <p className="text-[11px] text-blue-700/70">Growth state within professional valuation range</p>
                  </div>
                </div>
              </div>
            </div>
