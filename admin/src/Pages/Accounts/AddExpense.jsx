...
import {
  MOBILE_PROVIDERS as MOBILE_BANKING_PROVIDERS,
  BANK_OPTIONS,
...
export default function AddExpense() {
      ...
      .then((res) =>
        setCategoryOptions(
          (res.data.options || []).filter(
            (c) => !EXPENSE_CATEGORIES.includes(c),
          ),
        ),
      )
  ...
  const activityFilterCategories = [
    ...new Set(recentExpenses.map((t) => t.category).filter(Boolean)),
  ];

  const thisMonth = new Date().toISOString().slice(0, 7);
  const monthExpenses = (accounts?.transactions || []).filter(
  ...
  const todaysExpense = (accounts?.transactions || [])
    .filter(
      (t) =>
        t.type === "expense" && new Date(t.date).toDateString() === todayStr,
    )
    .reduce((s, t) => s + t.amount, 0);
  ...
  const weeklyPct =
    monthTotal > 0
      ? Math.min(Math.round((weeklyTotal / monthTotal) * 100), 100)
      : 0;

  const expenseRatio =
    accounts?.totalIncome > 0
      ? Math.min(
          Math.round((accounts.totalExpense / accounts.totalIncome) * 100),
          100,
        )
      : 0;
  const ratioStatus =
    expenseRatio < 50 ? "Good" : expenseRatio < 80 ? "Watch" : "High";
  const ratioColor =
    expenseRatio < 50 ? "#10B981" : expenseRatio < 80 ? "#F59E0B" : "#EF4444";
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between mb-6 sm:mb-8">
          <div>
            ...
            <p className="text-[#64748B] text-sm sm:text-[15px] mt-1">
              Record purchases, salaries, rent, operational costs and outgoing
              payments
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-5 mb-6 sm:mb-8">
          ...
          <div className="rounded-[12px] p-5 bg-white border border-[#E2E8F0] shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)]">
            ...
            <p className="text-[24px] sm:text-[26px] font-bold text-[#0F172A] mt-2 tracking-tight">
          ...
          <div className="rounded-[12px] p-5 bg-white border border-[#E2E8F0] shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)]">
            ...
            <Sparkline
              fillId="expMonthFill"
              color="#F87171"
              points="0,26 15,22 30,28 45,18 60,20 75,10 90,16 105,8 120,14"
            />
          </div>
          ...
          <div className="space-y-5">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                          <span className="text-[11px] bg-red-50 text-[#EF4444] px-2 py-0.5 rounded-full font-semibold">
                            {t.category}
                          </span>
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-[12px] shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)] p-5">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-[11px] font-bold text-[#EF4444] bg-red-50 px-2 py-0.5 rounded-full">
                  {weeklyPct}%
                </span>
              </div>
