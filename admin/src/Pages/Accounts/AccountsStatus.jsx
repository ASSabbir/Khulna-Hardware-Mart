...
import {
  ...
  FiSmartphone, FiCreditCard, FiSun, FiDownload,
  FiRefreshCw, FiChevronDown, FiSearch, FiSliders,
  FiZap, FiAlertTriangle, FiAward, FiCpu, FiTarget,
} from "react-icons/fi";
import {
...
const METHODS = {
  cash:   { label: "Cash",           icon: FiDollarSign, color: "#10B981", bg: "bg-emerald-50", text: "text-emerald-600" },
  mobile: { label: "Mobile Banking", icon: FiSmartphone,  color: "#8B5CF6", bg: "bg-purple-50",  text: "text-purple-600"  },
  bank:   { label: "Bank Account",   icon: FiCreditCard,  color: "#3B82F6", bg: "bg-blue-50",    text: "text-blue-600" },
};
...
function buildForecast(txns) {
  ...
  const avgNet = validNets.length ? validNets.reduce((a, b) => a + b, 0) / validNets.length : 0;
  ...
  for (let i = 1; i <= 3; i++) {
    const d = new Date(now.getFullYear(), now.getMonth() + i, 1);
    running += avgNet;
    forecast.push({ label: MONTH_NAMES[d.getMonth()], net: Math.round(running), projected: true });
  }
    ...
    <div className="bg-white border border-[#E2E8F0] rounded-[12px] p-5 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.04)]">
      ...
      {sparkPoints && (
        <svg viewBox="0 0 120 32" className="w-full h-8 mt-2" preserveAspectRatio="none">
          ...
          <polygon points={`0,32 ${sparkPoints} 120,32`} fill={`url(#${sparkId})`} />
          <polyline points={sparkPoints} fill="none" stroke={sparkColor} strokeWidth="2" strokeLinecap="round" />
        </svg>
      )}
      {extra}
    </div>
...
function CircularGauge({ value, size = 60, stroke = 7, color = "#10B981", track = "#F1F5F9" }) {
...
export default function AccountsStatus() {
  ...
  const loadData = () => {
    ...
    return axios.get(API_URL)
      ...
      .catch(() => setAccounts((prev) => prev || { balance: 0, totalIncome: 0, totalExpense: 0, transactions: [] }))
  ...
  useEffect(() => {
    ...
    const close = () => { setRangeOpen(false); setTxnFilterOpen(false); };
  ...
  const recent = useMemo(() =>
    [...txns].sort((a, b) => (b.datetime || b.date).localeCompare(a.datetime || a.date)).slice(0, 25), [txns]);

  const filteredRecent = useMemo(() => recent.filter((t) => {
  ...
  }).slice(0, 8), [recent, txnSearch, txnTypeFilter]);
  ...
  const growthRate = useMemo(() => {
    ...
    const thisIncome = txns.filter((t) => t.type === "income" && t.date.startsWith(thisM)).reduce((s, t) => s + t.amount, 0);
    const prevIncome = txns.filter((t) => t.type === "income" && t.date.startsWith(prevM)).reduce((s, t) => s + t.amount, 0);
  ...
  const expenseRatio = allIncome > 0 ? Math.round((allExpense / allIncome) * 100) : 0;
  const healthScore = Math.max(0, Math.min(100, 100 - expenseRatio));
  const riskLevel = expenseRatio >= 80 ? "High Risk" : expenseRatio >= 55 ? "Moderate Risk" : "Low Risk";
  const riskColor = expenseRatio >= 80 ? "#EF4444" : expenseRatio >= 55 ? "#F59E0B" : "#10B981";
  ...
  const exportCSV = () => {
    const rows = [["Date", "Type", "Category", "Method", "Description", "Added By", "Amount"]];
    recent.forEach((t) => rows.push([fmtDate(t.date), t.type, t.category, t.method, t.description, t.addedBy, t.amount]));
    ...
    <div className="min-h-screen bg-[#F8FAFC] font-sans">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-4 mb-6 sm:mb-8">
          <div>
            <h1 className="text-2xl sm:text-[28px] font-bold text-[#0F172A] tracking-tight">Account Overview</h1>
            <div className="flex items-center gap-1.5 mt-2 flex-wrap">
              ...
              <span className="text-[#94A3B8] text-xs">Last updated at {fmtTime(lastUpdated)}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
            <div className="relative" onClick={(e) => e.stopPropagation()}>
              <button type="button" onClick={() => setRangeOpen((o) => !o)}
                ...
                <FiCalendar size={15} className="text-[#64748B]" /> {rangeLabel} <FiChevronDown size={14} className="text-[#64748B]" />
              </button>
              {rangeOpen && (
                <div className="absolute z-20 right-0 mt-2 w-40 bg-white border border-[#E2E8F0] rounded-[12px] shadow-[0_10px_30px_-5px_rgba(0,0,0,0.15)] overflow-hidden">
                  {["This Month", "Last Month", "This Year", "All Time"].map((label) => (
                    <button key={label} type="button"
                      onClick={() => {
                        setRangeLabel(label);
                        const now = new Date();
                        if (label === "This Month") { setPeriod("monthly"); setSelectedMonth(todayStr.slice(0, 7)); }
                        if (label === "Last Month") {
                          const d = new Date(now.getFullYear(), now.getMonth() - 1, 1);
                          setPeriod("monthly"); setSelectedMonth(`${d.getFullYear()}-${pad2(d.getMonth() + 1)}`);
                        }
                        if (label === "This Year") { setPeriod("yearly"); setSelectedYear(now.getFullYear()); }
                        if (label === "All Time") { setPeriod("yearly"); setSelectedYear(now.getFullYear()); }
                        setRangeOpen(false);
                      }}
            ...
            <button type="button" onClick={loadData} disabled={refreshing}
              ...
              <FiRefreshCw size={15} className={`text-[#64748B] ${refreshing ? "animate-spin" : ""}`} />
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6 gap-4 mb-6 sm:mb-8">
          ...
          <KpiCard label="Total Expense" value={fmt(allExpense)} sub="Lifetime expense" trend="+3.2%" trendUp={false}
            sparkPoints="0,10 20,18 40,14 60,24 80,20 100,28 120,24" sparkColor="#EF4444" sparkId="sparkExp" />
          <KpiCard label={netProfit >= 0 ? "Net Profit" : "Net Loss"} value={fmt(Math.abs(netProfit))}
            sub={netProfit >= 0 ? "Profit" : "Loss"}
            extra={
              <span className={`inline-block mt-2 text-[10px] font-bold px-2 py-0.5 rounded-full ${netProfit >= 0 ? "bg-emerald-50 text-[#10B981]" : "bg-red-50 text-[#EF4444]"}`}>
                {netProfit >= 0 ? "GOOD" : "REVIEW"}
              </span>
            } />
          <div className="bg-white border border-[#E2E8F0] rounded-[12px] p-5 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.04)]">
            <p className="text-[#64748B] text-sm font-medium">Monthly Cash Flow</p>
            <p className="text-[13px] text-[#0F172A] mt-1.5 font-semibold">Income {fmt(monthStats.income)} / Expense {fmt(monthStats.expense)}</p>
            <div className="flex items-end gap-1.5 h-10 mt-2">
              <div className="flex-1 bg-emerald-100 rounded-t" style={{ height: `${Math.min((monthStats.income / (Math.max(monthStats.income, monthStats.expense, 1))) * 100, 100)}%` }} />
              <div className="flex-1 bg-red-100 rounded-t" style={{ height: `${Math.min((monthStats.expense / (Math.max(monthStats.income, monthStats.expense, 1))) * 100, 100)}%` }} />
            </div>
          </div>
          <div className="bg-white border border-[#E2E8F0] rounded-[12px] p-5 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.04)]">
            <p className="text-[#64748B] text-sm font-medium">Transaction Count</p>
            <p className="text-[22px] sm:text-[24px] font-bold text-[#0F172A] mt-1.5 tracking-tight">{(accounts?.transactions?.length || 0).toLocaleString()}</p>
            <p className="text-[#94A3B8] text-[11px] mt-1">All-time transactions</p>
            <div className="flex items-end gap-0.5 h-6 mt-2">
              {monthlyChart.map((m, i) => (
                <div key={i} className="flex-1 bg-blue-200 rounded-sm" style={{ height: `${Math.max((m.income + m.expense > 0 ? 30 : 5), Math.min(((m.income + m.expense) / (allIncome + allExpense || 1)) * 400, 100))}%` }} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 sm:mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-lg sm:text-xl font-bold text-[#0F172A]">Accounts</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {Object.entries(METHODS).map(([key, cfg]) => {
              ...
              const Icon = cfg.icon;
              return (
                <div key={key} className="bg-white border border-[#E2E8F0] rounded-[16px] p-5 sm:p-6 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.04)]">
                  <div className="flex items-center justify-between mb-4">
                    <span className="flex items-center gap-2 text-[#64748B] text-sm font-semibold">
                      <span className={`w-9 h-9 rounded-xl flex items-center justify-center ${cfg.bg}`}>
        ...
        <div className="grid grid-cols-1 2xl:grid-cols-[1fr_360px] gap-5 sm:gap-6">
          <div className="space-y-6">
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-[16px] p-5 sm:p-6 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.04)]">
              <h2 className="text-lg font-bold text-[#0F172A] mb-4">Smart Analytics Center</h2>
                  ...
                  {[
                    ...
                    <button key={key} onClick={() => setPeriod(key)}
                      className={`flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-sm font-semibold transition ${
                        period === key ? "bg-[#0F172A] text-white" : "text-[#64748B] hover:text-[#0F172A]"
                      }`}>
                      <Icon size={13} /> {label}
                    </button>
                  ))}
              ...
              <div className="h-72 sm:h-80 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={activeChart} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                    ...
                    <Bar dataKey="income" name="Income" fill="#10B981" radius={[4, 4, 0, 0]} barSize={period === "daily" ? 8 : 14} />
                    <Bar dataKey="expense" name="Expense" fill="#EF4444" radius={[4, 4, 0, 0]} barSize={period === "daily" ? 8 : 14} />
                    {showNetLine && (
                      <Line type="monotone" dataKey="net" name="Net Growth" stroke="#3B82F6" strokeWidth={2.5} dot={false} />
                    )}
                  </ComposedChart>
                </ResponsiveContainer>
              </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-lg font-bold text-[#0F172A]">Category Analytics</h2>
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                <div className="bg-white border border-[#E2E8F0] rounded-[16px] p-5 sm:p-6 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.04)]">
                  <div className="flex items-center gap-2 mb-5">
                    <FiPieChart size={16} className="text-[#10B981]" />
                    <h3 className="text-base font-bold text-[#0F172A]">Top Income Sources</h3>
                    <span className="ml-auto text-xs text-[#94A3B8] font-medium">{selectedMonth}</span>
                  </div>
                  {topIncome.length === 0 ? (
                    <p className="text-[#94A3B8] text-sm text-center py-8">No income this month</p>
                  ) : (
                        ...
                        return (
                          <div key={cat}>
                            <div className="flex items-center justify-between mb-1.5">
                              <span className="text-sm font-semibold text-[#334155]">{cat}</span>
                              <div className="flex items-center gap-3">
                                <span className="text-xs text-[#94A3B8]">{pct}%</span>
                                <span className="text-sm font-bold text-[#10B981]">{fmt(val)}</span>
                              </div>
                            </div>
                ...
                <div className="bg-white border border-[#E2E8F0] rounded-[16px] p-5 sm:p-6 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.04)]">
                  <div className="flex items-center gap-2 mb-5">
                    ...
                    <h3 className="text-base font-bold text-[#0F172A]">Top Expense Categories</h3>
                    <span className="ml-auto text-xs text-[#94A3B8] font-medium">{selectedMonth}</span>
                  </div>
                  {topExpense.length === 0 ? (
                    <p className="text-[#94A3B8] text-sm text-center py-8">No expenses this month</p>
                  ) : (
                    <div className="space-y-4">
                        ...
                        return (
                          <div key={cat}>
                            <div className="flex items-center justify-between mb-1.5">
                              ...
                              <div className="flex items-center gap-3">
                                <span className="text-xs text-[#94A3B8]">{pct}%</span>
                                <span className="text-sm font-bold text-[#EF4444]">{fmt(val)}</span>
                              </div>
                            </div>
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-[16px] overflow-hidden shadow-[0_4px_20px_-2px_rgba(15,23,42,0.04)]">
              <div className="px-5 sm:px-6 py-5 border-b border-[#F1F5F9] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-base font-bold text-[#0F172A]">Recent Transactions Center</h2>
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <div className="relative" onClick={(e) => e.stopPropagation()}>
                    <button type="button" onClick={() => setTxnFilterOpen((o) => !o)}
                      className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-lg border transition ${
                        txnTypeFilter ? "border-[#3B82F6] text-[#3B82F6] bg-blue-50" : "border-[#E2E8F0] text-[#64748B] hover:bg-[#F8FAFC]"
                      }`}>
                      <FiSliders size={13} /> Filter
                    </button>
                    {txnFilterOpen && (
                      <div className="absolute z-20 right-0 mt-2 w-40 bg-white border border-[#E2E8F0] rounded-[12px] shadow-[0_10px_30px_-5px_rgba(0,0,0,0.15)] overflow-hidden">
                        {[["", "All Types"], ["income", "Income"], ["expense", "Expense"]].map(([v, l]) => (
                          <button key={v} type="button" onClick={() => { setTxnTypeFilter(v); setTxnFilterOpen(false); }}
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-[#F1F5F9]">
                    {filteredRecent.length === 0 ? (
                      <tr><td colSpan={7} className="text-center py-16 text-[#94A3B8] text-sm">No transactions found</td></tr>
                      ...
                      <tr key={t.id} className="hover:bg-[#F8FAFC] transition">
                        <td className="px-5 py-4 text-sm text-[#64748B] whitespace-nowrap">
                          <span className="flex items-center gap-1.5"><FiCalendar size={12} className="text-[#CBD5E1]" />{fmtDate(t.date)}</span>
                        </td>
                        <td className="px-5 py-4">
                          <span className={`inline-flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1 rounded-full ${t.type === "income" ? "bg-emerald-50 text-[#10B981]" : "bg-red-50 text-[#EF4444]"}`}>
                            {t.type === "income" ? <FiArrowUpRight size={11} /> : <FiArrowDownRight size={11} />}
                            {t.type === "income" ? "Income" : "Expense"}
                          </span>
                        </td>
                        <td className="px-5 py-4">
                          <span className="text-[11px] font-semibold bg-[#F1F5F9] text-[#475569] px-2.5 py-1 rounded-lg">{t.category}</span>
                        </td>
                        <td className="px-5 py-4">
                          <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-lg ${METHODS[t.method]?.bg} ${METHODS[t.method]?.text}`}>
                        ...
                        <td className="px-5 py-4 text-sm text-[#64748B]">{t.addedBy}</td>
                        <td className="px-5 py-4">
              ...
              <div className="md:hidden divide-y divide-[#F1F5F9]">
                {filteredRecent.length === 0 ? (
                  <p className="text-center text-[#94A3B8] text-sm py-10">No transactions found</p>
                ) : filteredRecent.map((t) => (
                  <div key={t.id} className="px-5 py-4 flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3">
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${t.type === "income" ? "bg-emerald-50" : "bg-red-50"}`}>
                        {t.type === "income" ? <FiArrowUpRight size={16} className="text-[#10B981]" /> : <FiArrowDownRight size={16} className="text-[#EF4444]" />}
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-[#0F172A]">{t.description}</p>
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                          <span className="text-[11px] bg-[#F1F5F9] text-[#64748B] px-2 py-0.5 rounded-md font-medium">{t.category}</span>
                          <span className={`text-[11px] px-2 py-0.5 rounded-md font-medium ${METHODS[t.method]?.bg} ${METHODS[t.method]?.text}`}>
                    ...
                    <span className={`text-sm font-bold whitespace-nowrap ${t.type === "income" ? "text-[#10B981]" : "text-[#EF4444]"}`}>
                      {t.type === "income" ? "+" : "-"}{fmt(t.amount)}
                    </span>
                  </div>
                ))}
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-[16px] p-5 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.04)]">
              <p className="text-sm font-bold text-[#0F172A] mb-1">Cash Flow Forecast</p>
              <p className="text-[11px] text-[#94A3B8] mb-3">Next 3 months, based on 6-month average</p>
              <div className="h-24">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={forecastData} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
                    <defs>
                    ...
                    <Tooltip content={<ChartTooltip />} />
                    <Area type="monotone" dataKey="net" name="Net" stroke="#3B82F6" fill="url(#forecastFill)" strokeWidth={2} dot={false} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-[16px] p-5 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.04)] flex items-center gap-4">
              <CircularGauge value={Math.min(Math.abs(growthRate), 100)} color={growthRate >= 0 ? "#10B981" : "#EF4444"} />
              <div>
                <p className="text-sm font-bold text-[#0F172A]">Monthly Growth Rate</p>
                <p className={`text-xl font-bold mt-0.5 ${growthRate >= 0 ? "text-[#10B981]" : "text-[#EF4444]"}`}>
                  {growthRate >= 0 ? "+" : ""}{growthRate}%
                </p>
                <p className="text-[11px] text-[#94A3B8] mt-0.5">Income vs last month</p>
              </div>
            </div>
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-[16px] p-5 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.04)] flex items-start gap-3"
              ...
              <span className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${riskColor}1A` }}>
                <FiAlertTriangle size={16} style={{ color: riskColor }} />
              </span>
              <div>
                <p className="text-sm font-bold text-[#0F172A]">Expense Risk Alert</p>
                <p className="text-xl font-bold mt-0.5" style={{ color: riskColor }}>{riskLevel}</p>
                <p className="text-[11px] text-[#94A3B8] mt-1">Expenses are {expenseRatio}% of total income</p>
              </div>
            </div>
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-[16px] p-5 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.04)] flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-[#0F172A]">Top Performing Income Source</p>
                {topIncome[0] ? (
                  <>
                    <p className="text-lg font-bold text-[#10B981] mt-0.5">{topIncome[0][0]}</p>
                    <p className="text-[11px] text-[#94A3B8] mt-0.5">{fmt(topIncome[0][1])} this month</p>
                  </>
                ) : (
                  <p className="text-[11px] text-[#94A3B8] mt-1">No income recorded this month</p>
                )}
              </div>
            </div>
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-[16px] p-5 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.04)] flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-[#0F172A]">Largest Expense Category</p>
                {topExpense[0] ? (
                  <>
                    <p className="text-lg font-bold text-[#EF4444] mt-0.5">{topExpense[0][0]}</p>
                    <p className="text-[11px] text-[#94A3B8] mt-0.5">{fmt(topExpense[0][1])} this month</p>
                  </>
                ) : (
                  <p className="text-[11px] text-[#94A3B8] mt-1">No expenses recorded this month</p>
                )}
              </div>
            </div>
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-[16px] p-5 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.04)]">
              <p className="text-sm font-bold text-[#0F172A] mb-2">Weekly Performance Snapshot</p>
              <div className="flex items-center justify-between text-xs mb-1">
                <span className="text-[#64748B]">Income</span>
                <span className="font-bold text-[#10B981]">{fmt(weeklyStats.income)}</span>
              </div>
              <div className="flex items-center justify-between text-xs mb-3">
                <span className="text-[#64748B]">Expense</span>
                <span className="font-bold text-[#EF4444]">{fmt(weeklyStats.expense)}</span>
              </div>
              <p className={`text-lg font-bold ${weeklyStats.profit >= 0 ? "text-[#10B981]" : "text-[#EF4444]"}`}>
                {weeklyStats.profit >= 0 ? "+" : ""}{fmt(weeklyStats.profit)}
              </p>
              <p className="text-[11px] text-[#94A3B8] mt-0.5">Net over last 7 days</p>
            </div>

            {/* AI Financial Insights Panel */}
            <div className="bg-gradient-to-br from-blue-50 to-white border border-blue-100 rounded-[16px] p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-sm font-bold text-[#0F172A]">AI Financial Insights</p>
              </div>
              <ul className="space-y-2 text-xs text-[#475569] leading-relaxed">
                <li className="flex gap-2">
                  <FiZap size={12} className="text-[#3B82F6] mt-0.5 flex-shrink-0" />
                  {expenseRatio >= 55
        ...
        <p className="text-center text-[#94A3B8] text-sm pt-6 pb-4">BuildMart Hardware — Account Overview</p>
      </div>
    </div>
  );
}