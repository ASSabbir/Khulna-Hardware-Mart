...
function Toast({ msg, type, onClose }) {
  ...
  return (
    <div
      className={`fixed top-6 right-6 z-50 flex items-center gap-3 px-5 py-4 rounded-2xl shadow-xl text-white text-base font-medium transition-all
...
function Card({ children, className = "" }) {
  return (
    <div
      className={`bg-white border border-gray-200 rounded-3xl p-7 shadow-sm ${className}`}
    >
      {children}
    </div>
  );
    ...
    <div className="mb-5">
      <div className="flex gap-2 mb-2">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className={`h-2 flex-1 rounded-full transition-all ${i <= score ? colors[score] : "bg-gray-200"}`}
          />
        ))}
      </div>
      {password && (
        <p
          className={`text-base font-medium ${score <= 1 ? "text-red-500" : score === 2 ? "text-yellow-600" : score === 3 ? "text-blue-600" : "text-green-600"}`}
        >
          {labels[score]}
        </p>
      )}
      <ul className="mt-2 space-y-1">
        {[
          ...
          <li
            key={text}
            className={`flex items-center gap-2 text-base ${ok ? "text-green-600" : "text-gray-400"}`}
          >
...
const AdminDeshboard = () => {
  ...
  const handleChangePassword = () => {
    if (!cpForm.current || !cpForm.newPwd || !cpForm.confirm)
      return showToast("Fill all fields, boss! 😤", "error");
    if (cpForm.newPwd !== cpForm.confirm)
      return showToast("Passwords don't match! Try again 🔁", "error");
    if (cpForm.newPwd.length < 8)
      return showToast("Password too short! Min 8 chars 🔐", "error");
    showToast("Password changed! Don't forget it this time! 😄", "success");
    setCpForm({ current: "", newPwd: "", confirm: "" });
  };

  const handleAddAdmin = () => {
    if (!naForm.name || !naForm.email || !naForm.password)
      return showToast("Fill all fields! 📝", "error");
    if (!naForm.email.includes("@"))
      return showToast("That's not an email! 😂", "error");
    if (admins.find((a) => a.email === naForm.email))
      return showToast("This email already exists! 👀", "error");
    const newAdmin = {
      id: Date.now(),
      name: naForm.name,
      email: naForm.email,
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center px-4">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
            ...
            <p className="text-gray-500 text-base mb-6">
              ...
              <strong>
                {admins.find((a) => a.id === deleteConfirm)?.name}
              </strong>
              . They're outta here! 👋
            </p>
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 border border-gray-200 text-gray-700 font-semibold py-3 rounded-xl text-base hover:bg-gray-50 transition"
              >
                Cancel
              </button>
              <button
                onClick={() => handleDelete(deleteConfirm)}
                className="flex-1 bg-red-500 hover:bg-red-600 text-white font-semibold py-3 rounded-xl text-base transition"
              >
                Yes, Remove
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
            <div>
              <PwdInput
                ...
                onChange={(e) =>
                  setCpForm({ ...cpForm, current: e.target.value })
                }
              ...
              <PwdInput
                ...
                onChange={(e) =>
                  setCpForm({ ...cpForm, confirm: e.target.value })
                }
            ...
            <div className="mt-6 md:mt-9">
              ...
              <div className="mt-6 bg-amber-50 border border-amber-200 rounded-2xl p-4">
                <p className="text-amber-800 text-base font-semibold flex items-center gap-2">
                  <FiAlertTriangle size={16} /> Pro tips:
                </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
            <div>
              ...
              <Input
                ...
                value={naForm.email}
                onChange={(e) =>
                  setNaForm({ ...naForm, email: e.target.value })
                }
              ...
              <PwdInput
                ...
                onChange={(e) =>
                  setNaForm({ ...naForm, password: e.target.value })
                }
            ...
            <div className="mt-6 md:mt-0">
              <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5 mb-4">
                <p className="text-blue-800 text-2xl font-bold mb-2">
                  {" "}
                  Role Permissions
                </p>
              ...
              <div className="bg-green-50 border border-green-200 rounded-2xl p-4">
                <p className="text-green-800 text-base">
                  New admins get an email invite and can login right away. They
        ...
        <Card>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    ...
                    {admin.you && (
                      <span className="bg-green-600 text-white text-base px-3 py-0.5 rounded-full font-semibold">
                        You{" "}
                      </span>
                    )}
                    <span
                      className={`text-base px-3 py-0.5 rounded-full font-semibold ${admin.role === "Super Admin" ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"}`}
                    >
                  ...
                  {!admin.you && (
                    <>
                      <button
                        onClick={() =>
                          showToast(
                            `Edit for ${admin.name} — coming soon! 🔧`,
                            "success",
                          )
                        }
          ...
          <div className="mt-6 bg-orange-50 border border-orange-200 rounded-2xl p-4 flex items-start gap-3">
            ...
            <p className="text-orange-800 text-base">
              ...
              other admins. Don't give keys to strangers!
            </p>
          </div>
        </Card>
      </div>
