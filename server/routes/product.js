// FILE: server/routes/product.js (FULL REPLACEMENT)
...
// Creates a Product document, automatically regenerating the SKU and retrying
// if a duplicate-key collision happens (e.g. a race between two simultaneous saves).
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      if (error.code === 11000 && error.keyPattern && error.keyPattern.sku) {
        lastError = error;
        continue; // regenerate a fresh SKU and try again
      }
      throw error;
    }
  }
...
// suppliers[].availableQuantity was being set once at purchase/restock time and
// never decremented when stock sold via deductStockFIFO (which only touches
// batches[].quantity) — that's why the dropdown numbers drift from real stock.
// This recalculates availableQuantity per supplier straight from the batches
// array (the actual source of truth), so it can never go out of sync.
function syncSupplierAvailableQuantities(product) {
  ...
  product.suppliers.forEach((s) => {
    s.availableQuantity = bySupplier[String(s.supplierId)] || 0;
  });
}
...
async function updateSupplierStats(supplierId, price, qty, session) {
  ...
  await Supplier.findByIdAndUpdate(
    ...
    {
      $inc: {
        totalPurchaseAmount: Number(price) * Number(qty),
        totalPurchaseCount: 1,
        totalPurchasedQuantity: Number(qty),
      },
      $set: {
        lastPurchasePrice: Number(price),
    ...
    const breakdown = products.map((p) => {
      ...
      return {
        ...
        batches: batches.map((b) => ({
          ...
          quantity: b.quantity,
          value: (b.quantity || 0) * (b.buyingPrice || 0),
        })),
      };
    });
    res.json({ totalValue, products: breakdown });
  } catch (error) {
...
// Create Product — stock is fully derived from the `suppliers` array (each entry
// is one purchase batch: supplier + buying price + quantity + purchase date).
// This is the single source of truth for opening stock, so there is no separate
// "opening stock" number that could conflict or double-count.
router.post("/", async (req, res) => {
  try {
    ...
    if (supplierEntries.length === 0) {
      return res.status(400).json({ message: "Add at least one supplier with a purchase quantity to set opening stock." });
    }
    if (supplierEntries.length > 20) {
      return res.status(400).json({ message: "Too many supplier entries (max 20)." });
    }
    ...
    for (const s of supplierEntries) {
      ...
      totalStock += qty;
      resolvedEntries.push({ resolved, price, qty, purchaseDate });
    }
    ...
    for (const entry of resolvedEntries) {
      await PurchaseHistory.create({
        ...
        supplierId: entry.resolved.supplierId,
        supplierName: entry.resolved.supplierName,
        buyingPrice: entry.price,
        quantity: entry.qty,
...
async function createCustomProduct({ name, unitPrice, qty }) {
  const category = "Other";
  const batch = {
    supplierId: null,
    supplierName: "N/A (Custom Product)",
    buyingPrice: Number(unitPrice) || 0,
    quantity: Number(qty) || 0,
    originalQuantity: Number(qty) || 0,
  ...
  return createProductWithUniqueSKU(
    {
      ...
      unit: "pcs",
      buyingPrice: Number(unitPrice) || 0,
      holcellPrice: Number(unitPrice) || 0,
      retailPrice: Number(unitPrice) || 0,
      stock: Number(qty) || 0,
      reorderLevel: 0,
      status: "active",
      isCustom: true,
...
router.put("/:id", async (req, res) => {
  try {
    ...
    // Custom → regular conversion: replace the placeholder "N/A (Custom Product)"
    // supplier on every batch/supplier entry with a real, resolved supplier.
    if (body.isCustom === false && product.isCustom && supplierAssignment) {
      const resolved = await resolveSupplier(supplierAssignment);
      product.batches.forEach((b) => {
        b.supplierId = resolved.supplierId;
        b.supplierName = resolved.supplierName;
      });
      product.suppliers = [{
        supplierId: resolved.supplierId,
        supplierName: resolved.supplierName,
        buyingPrice: body.buyingPrice ?? product.buyingPrice,
        lastPurchaseDate: new Date(),
        availableQuantity: 0,
      }];
...
router.post("/:id/restock", async (req, res) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) return res.status(400).json({ message: "Invalid product id." });
    const { supplierId, supplierName, buyingPrice, quantity, purchaseDate } = req.body;
...
async function restoreStockFIFO(productId, qtyToRestore, session, batchRefs = [], originalSoldQty = null) {
  ...
  if (Array.isArray(batchRefs) && batchRefs.length > 0) {
    ...
    if (totalSold > 0) {
      ...
      for (let i = 0; i < batchRefs.length; i++) {
        ...
        if (batch) {
          batch.quantity += share;
        } else {
          product.batches.push({
            batchId: ref.batchId || undefined,
            supplierId: ref.supplierId || null,
            supplierName: ref.supplierName || "Return Restock",
            buyingPrice: ref.buyingPrice ?? product.buyingPrice ?? 0,
            quantity: share,
            originalQuantity: share,
            purchaseDate: new Date(),
          });
        }
        allocated += share;
  ...
  if (remaining > 0) {
    if (product.batches.length > 0) {
      product.batches[product.batches.length - 1].quantity += remaining;
    } else {
      product.batches.push({
        supplierId: null,
        supplierName: "Return Restock",
